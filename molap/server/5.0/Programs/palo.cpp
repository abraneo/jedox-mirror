...
static void linuxCrashHandler(int signal)
{
	...
	if (z) {
		FileUtils::addToZip(z, fileName + ".txt");
		FileUtils::addToZip(z, DumpHandler::m_palo_ini);
...
static int startPalo(PaloOptions& options, callback sink)
{
	...
	try {
		...
		// create server
		if (createServer) {
			createServer(FileName(options.dataDirectory, "palo", "csv"));
		} else {
			Server::create(FileName(options.dataDirectory, "palo", "csv"));
		}
