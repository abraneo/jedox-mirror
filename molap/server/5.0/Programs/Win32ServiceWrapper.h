...
public:
	...
	static void InstallServiceCommand(PaloOptions& options, const string& command) {
		...
		if (vi.dwPlatformId == VER_PLATFORM_WIN32_NT && vi.dwMajorVersion >= 6) {
			...
			ChangeServiceConfig2A(schService, SERVICE_CONFIG_DELAYED_AUTO_START_INFO, &info);
		}*/
	...
	static void WINAPI service_main(DWORD dwArgc, LPSTR *lpszArgv) {
		...
		service_status = RegisterServiceCtrlHandlerA(lpszArgv[0], (LPHANDLER_FUNCTION) service_ctrl);
