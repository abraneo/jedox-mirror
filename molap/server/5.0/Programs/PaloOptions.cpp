...
static const char * AllowedOptions[] = {"?|help",
        ...
        "G?ignore-rule-error", //it should be "G|ignore-rule-error" but G? is used due to backward compatibility
        "h+http                  <address> <port>",
	...
	arguments(argc - 1, argv + 1), options(*argv, AllowedOptions)
{
	...
	cacheClearBarrierCells = 100000.0;
...
void PaloOptions::usage()
{
	...
	if (showUsage) {
		cout << "data-directory:        " << dataDirectory << "\n"
		     ...
		     << "ignore-rule-error:     " << (ignoreRuleError ? "true" : "false") << "\n"
		...
		try {
			...
			case 'G':
				if (optarg.size()) {
					...
					ignoreRuleError = !ignoreRuleError;
				}
				break;
