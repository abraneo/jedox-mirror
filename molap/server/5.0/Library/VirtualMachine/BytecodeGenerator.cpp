...
void bytecode_generator::GrowDblStackCode()
{
	m_rule.gc_dbl_stack_nr++;
	if (m_rule.gc_dbl_stack_max < m_rule.gc_dbl_stack_nr) {
		m_rule.gc_dbl_stack_max = m_rule.gc_dbl_stack_nr;
	}
}

void bytecode_generator::ShrinkDblStackCode()
{
	m_rule.gc_dbl_stack_nr--;
}

void bytecode_generator::GrowStrStackCode()
{
	m_rule.gc_str_stack_nr++;
	if (m_rule.gc_str_stack_max < m_rule.gc_str_stack_nr) {
		m_rule.gc_str_stack_max = m_rule.gc_str_stack_nr;
	}
}

void bytecode_generator::ShrinkStrStackCode()
{
	m_rule.gc_str_stack_nr--;
}
...
bool bytecode_generator::EmitCallDataDblCode(uint8_t params, uint8_t dims, int16_t contextCubeIndex, const int16_t* dimensionOrder, uint32_t *dimensionElements)
{
	...
	if (contextCubeIndex != -1) {
		...
		ShrinkStrStackCode();	// database name
		ShrinkStrStackCode();	// cube name
	}
	for (i = 0; i < dims; i++) {
		if (contextCubeIndex != -1) {
		...
		ShrinkStrStackCode();	// element name
	}
...
bool bytecode_generator::EmitIsError(uint8_t num)
{
	if (num) {
		if (!EmitCode(FUNC_ISERROR_DBL))
			return false;
		ShrinkDblStackCode();
		GrowDblStackCode();
	} else {
		if (!EmitCode(FUNC_ISERROR_STR))
			return false;
		ShrinkStrStackCode();
		GrowDblStackCode();
	}
