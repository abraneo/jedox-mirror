...
void Dimension::clearElements(PServer server, PDatabase db, PUser user, bool useDimWorker, bool useJournal)
{
	if (!protectedElems.empty()) {
		...
		checkElementAccessRight(user.get(), RIGHT_DELETE);
...
static bool isEnough(User::MapElementRight *elemRights, IdentifierType elemId, RightsType defRight)
{
	if (elemRights) {
		User::MapElementRight::const_iterator it = elemRights->find(elemId);
		if (it != elemRights->end()) {
			return it->second >= RIGHT_READ;
		}
	}
	return defRight >= RIGHT_READ;
}

ElementsType Dimension::getElements(PUser user, bool onlyRoot) const
{
	checkElementAccessRight(user.get(), RIGHT_READ);
	...
	User::MapElementRight *elemRights = 0;
	...
	for (IdentifierType position = 0; position < totalElements; position++) {
		...
		if (element && element->getElementType() != Element::UNDEFINED) {
			bool addElem = false;
			if (checkRights) {
				if (minRight >= RIGHT_READ || isEnough(elemRights, element->getIdentifier(), db->getDefaultRight())) {
					addElem = true;
					if (onlyRoot && element->getParentsCount()) {
						...
						for (Parents::const_iterator parentit = parentIds->begin(); parentit != parentIds->end(); ++parentit) {
							...
							if (minRight >= RIGHT_READ || isEnough(elemRights, *parentit, db->getDefaultRight())) {
								addElem = false;
								break;
							}
						}
...
void Dimension::moveElement(PServer server, PDatabase db, Element *element, PositionType newPosition, PUser user, bool useJournal)
{
	...
	checkElementAccessRight(user.get(), RIGHT_WRITE);
...
void Dimension::addElementEvent(PServer server, PDatabase db, Element *element)
{
	...
	if (worker) {
		...
		if (worker->triggerCreateElement(db->getId(), getId(), function)) {
			...
			ResultStatus status = worker->notifyElementCreated(db->getId(), getId(), element->getIdentifier(), function, session ? session->getSid() : "");
			if (status != RESULT_OK) {
...
void Dimension::addChildren(PServer server, PDatabase db, Element *parent, const IdentifiersWeightType *children, PUser user, CubeRulesArray* disabledRules, bool preserveOrder, bool updateElementInfo, bool useJournal, IdentifiersType *elemsToDeleteFromCubes)
{
	...
	if (doRemoveElement(oldType, oldISC, parent->getElementType(), parent->isStringConsolidation())) {
		...
		if (elemsToDeleteFromCubes && type == Dimension::DEL_NUM) {
			...
			removeElementFromCubes(server, db, user, parent->getIdentifier(), true, disabledRules, type, false);
		}
	}
...
void Dimension::removeChildren(PServer server, PDatabase db, PUser user, Element *element, CubeRulesArray* disabledRules, bool useJournal, bool changingToString)
{
	...
	if (oldISC) {
		...
		if (!changingToString) {
			removeElementFromCubes(server, db, user, element->getIdentifier(), true, disabledRules, getDeleteCellType(Element::CONSOLIDATED, oldISC), false);
		}
	...
	if (oldType == Element::CONSOLIDATED) {
		...
		if (doRemove && doRemoveElement(oldType, false, elementType, newISC)) {
			...
			if (type == Dimension::DEL_NUM && elemsToDeleteFromCubes) {
				...
				removeElementFromCubes(server, db, user, element->getIdentifier(), true, disabledRules, type, false);
			}
		}
	}
...
void Dimension::deleteElements(PServer server, PDatabase db, IdentifiersType elementsToDelete, PUser user, bool useJournal, CubeRulesArray* disabledRules, bool useDimWorker)
{
	...
	if (!deletedPositions.empty()) {
		...
		removeElementsFromCubes(server, db, user, elementsToDelete, true, disabledRules, DEL_ALL, true);
...
void Dimension::removeElementFromCubes(PServer server, PDatabase db, PUser user, IdentifierType element, bool deleteRules, CubeRulesArray* disabledRules, DeleteCellType delType, bool completeRemove)
{
	...
	for (vector<CPCube>::iterator c = cubes.begin(); c != cubes.end(); ++c) {
		...
		PCube cube = COMMITABLE_CAST(Cube, cubeList->get((*c)->getId(), true));
		context->saveParent(db, cube);
		if (cube->deleteElement(server, db, user, server->getEvent(), thisDimension, element, deleteRules, disabledRules, delType, completeRemove)) {
			cubeList->set(cube);
		}
	}
}

void Dimension::removeElementsFromCubes(PServer server, PDatabase db, PUser user, IdentifiersType elementIds, bool deleteRules, CubeRulesArray* disabledRules, DeleteCellType type, bool completeRemove)
{
	PDimension thisDimension = COMMITABLE_CAST(Dimension, shared_from_this());
	...
	for (CubeList::Iterator c = cl->begin(); c != cl->end(); ++c) {
		...
		context->saveParent(db, cube);
		if (cube->deleteElements(server, db, user, server->getEvent(), thisDimension, elementIds, deleteRules, disabledRules, type, fullSet, completeRemove)) {
			cl->set(cube);
		}
	}
}
	...
	if (hasRightsCube() && User::checkUser(user) && db->getHideElements()) {
		...
		if (parent) {
			...
			for (ElementsWeightType::const_iterator it = allChildren.begin(); it != allChildren.end(); ++it) {
				if (user->checkGroupDimensionDataRight(db, groupDimensionDataCube, dim, it->first->getIdentifier(), RIGHT_READ)) {
...
Element* Dimension::findElementByPosition(PositionType position, User *user, bool write) const
{
	checkElementAccessRight(user, RIGHT_READ);
...
void Dimension::checkHideElement(Element *element, User *user) const
{
	if (hasRightsCube() && User::checkUser(user)) {
		...
		CPCube groupDimensionDataCube = db->findCubeByName(SystemCube::PREFIX_GROUP_DIMENSION_DATA + dim->getName(), PUser(), true, false);
		if (db->getHideElements() && !(user->checkGroupDimensionDataRight(db, groupDimensionDataCube, dim, element->getIdentifier(), RIGHT_READ))) {
