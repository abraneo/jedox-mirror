...
#include "PaloGpu/GpuEngine/GpuEngine.h"
#endif
...
Server::Server(const Server & s) :
		Commitable(s), token(s.token), dataToken(s.dataToken), dbs(s.dbs), fileName(s.fileName), systemDatabase(s.systemDatabase), loginWorker(s.loginWorker), dimensionWorker(s.dimensionWorker), filelock(s.filelock), blocking(s.blocking), activeSessionSid(s.activeSessionSid), username(s.username), event(s.event), loginType(s.loginType), winSSOenabled(s.winSSOenabled), defaultTtl(s.defaultTtl), drillThroughEnabled(s.drillThroughEnabled), m_DisableMarkerCalculation(s.m_DisableMarkerCalculation), encryption(
		        s.encryption), shortSid(s.shortSid), enableGpu(s.enableGpu), engines(s.engines), bigEndian(s.bigEndian), buildMarkers(s.buildMarkers), tp(s.tp)
{
}
...
void Server::updateLicenses(PSystemDatabase sysdb)
{
	...
	PCellStream cs = userPropsCube->calculateArea(area, CubeArea::ALL, ALL_RULES, true, UNLIMITED_UNSORTED_PLAN);
...
void Server::updateGpuBins(PDimension dimension, PServer server, PaloJobRequest* jobRequest, PDatabase database, Context* context, PUser user)
{
	...
	if(gCubes.size() > 0){
		...
		PEngineBase engineGpu = server->getEngine(EngineBase::GPU, true);
		...
		for (size_t cubeIdx = 0; cubeIdx < gCubes.size(); ++cubeIdx){
			...
			if(dimPos[indexOfLastDimInActBin] >= numAdditionalBitsNeeded){ //if there's enough room in the actual bin for the additional element
				...
				storageGpu = engineGpu->getCreateStorage(id, pathTrans, EngineBase::Numeric);
				...
				IdentifierType id(cubecopy->getNumericStorageId());
				storageGpu = engineGpu->getCreateStorage(id, pathTransNew, EngineBase::Numeric);
				Logger::trace << "Resizing dimension " << dimIdx << " in bin " << actBin << " of cube " << gCubes[cubeIdx]->getId() << endl;
				if(addNewBin)
...
IdentifierType Server::loadServerDatabase(FileReader *file)
{
	...
	if (FileUtils::isReadable(dbFile)) {
		...
		addDatabase(database, false, false);
	} else {
...
vector<CPDatabase> Server::getDatabases(PUser user) const
{
	...
	for (DatabaseList::Iterator i = dbs->begin(); i != dbs->end(); ++i) {
		...
		if (database != 0) {
			if (database->getType() != SYSTEMTYPE || showSystem) {
...
void Server::addDatabase(PDatabase database, bool notify, bool newid)
{
	checkCheckedOut();
	...
	if (notify) {
		database->notifyAddDatabase(COMMITABLE_CAST(Server, shared_from_this()));
	}
...
// this method is used internally, so no rights checking is done
void Server::removeDatabase(PDatabase database, bool notify)
{
	checkCheckedOut();
	IdentifierType identifier = database->getIdentifier();
...
// this method is used internally, so no rights checking is done
void Server::renameDatabase(PDatabase database, const string & name, bool notify)
{
	checkCheckedOut();
...
PDatabase Server::addDatabase(const string & realName, PUser user, IdentifierType type)
{
	checkCheckedOut();
	...
	addDatabase(database, true, true);
	// return database
	return database;
}

void Server::deleteDatabase(PDatabase database, PUser user)
{
	checkCheckedOut();
...
void Server::renameDatabase(PDatabase database, const string & realName, PUser user)
{
	checkCheckedOut();
	checkDatabaseAccessRight(user, RIGHT_WRITE);
...
void Server::saveDatabase(PDatabase database, PUser user, bool sendEvent, string *backupZipPath)
{
	...
	if (backupZipPath) {
		...
		FileUtils::zipDirectory(database->getPath(), *backupZipPath);
	}
...
{
	checkCheckedOut();
	PDatabase database = lookupDatabaseByName(NAME_SYSTEM_DATABASE, false);
	if (database == 0) {
		Logger::info << "system database not found" << endl;

		// create system database
		systemDatabase = PSystemDatabase(new SystemDatabase(NAME_SYSTEM_DATABASE));
		systemDatabase->setDeletable(false);
		systemDatabase->setRenamable(false);
		database = systemDatabase;

		FileName dbFile = computeDatabaseFileName(fileName, NAME_SYSTEM_DATABASE);
		if (FileUtils::isReadable(dbFile)) {
			addDatabase(systemDatabase, false, true);
		} else {
			addDatabase(systemDatabase, true, true);

			Logger::trace << "system database check" << endl;
			// create system dimensions and cubes
			systemDatabase->createSystemItems(COMMITABLE_CAST(Server, shared_from_this()), true);
	...
	if (database->getType() == SYSTEMTYPE) {
		...
		systemDatabase->createSystemItems(COMMITABLE_CAST(Server, shared_from_this()));
	} else {
		Logger::error << "system database is not of type SYSTEM" << endl;
