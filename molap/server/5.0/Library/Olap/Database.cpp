...

	{
...

		{
			...
			if (addSystemDimension) {
				this->addSystemDimension(server, dbChanged); // calls NormalDatabase::addSystemDimension
			}
...
vector<CPDimension> Database::getDimensions(PUser user) const
{
	...
	for (DimensionList::Iterator i = dims->begin(); i != dims->end(); ++i) {
		...
		if (!User::checkUser(user)) {
			...
			PSystemDimension system = dimension->getType() == SYSTEMTYPE ? COMMITABLE_CAST(SystemDimension, dimension) : PSystemDimension();

			if (system) {
				switch (system->getDimensionType()) {
				...
				case Dimension::RIGHTS:
					if (user->getRoleRight(User::rightsRight) > RIGHT_NONE) {
						result.push_back(dimension);
					}
					break;

				case Dimension::CONFIG:
//					if (user->getRoleRight(User::sysOpRight) > RIGHT_NONE) {
						result.push_back(dimension);
//					}
					break;

				case Dimension::ALIAS:
					if (user->getRoleRight(User::rightsRight) > RIGHT_NONE || user->getRoleRight(User::subSetViewRight) > RIGHT_NONE) {
						result.push_back(dimension);
					}
					break;
				...
				if (dimension->getType() == USER_INFOTYPE) {
					// user info dimension
					if (user->getRoleRight(User::userInfoRight) > RIGHT_NONE) {
						result.push_back(dimension);
					}
				} else {
					// normal dimension
...
vector<CPCube> Database::getCubes(PUser user) const
{
	...
	for (CubeList::Iterator i = cubes->begin(); i != cubes->end(); ++i) {
		...
		if (!User::checkUser(user)) {
			...
			PSystemCube system = cube->getType() == SYSTEMTYPE ? COMMITABLE_CAST(SystemCube, cube) : PSystemCube();
			if (system) {
				// system cube

				switch (system->getCubeType()) {
...
//						}
					...
					case Cube::SUBSETVIEW:
						if (user->getRoleRight(User::subSetViewRight) > RIGHT_NONE) {
							result.push_back(cube);
						}
						break;
					default:
	...
	if (numericStorageId != NO_IDENTIFIER) {
		...
		if (type == GPUTYPE) {
			...
			if (numericGpuStorage) {
				...
				numericGpuStorage->setCellValue(numericCpuStorage->getCellValues(CPArea(), 0));
			}
            cube = cubecopy;
