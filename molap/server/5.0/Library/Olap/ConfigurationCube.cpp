...
void ConfigurationCube::clearCells(PServer server, PDatabase db, PUser user, bool useJournal)
{
	PDimension dim = db->lookupDimension(dimensions.at(0), false);
	ElementsType elements = dim->getElements(user, false);

	set<PCube> changedCubes;
	for (ElementsType::iterator i = elements.begin(); i != elements.end(); ++i) {
		// do not clear client cache cell
		string name = (*i)->getName(dim->getElemNamesVector());
		if (name != SystemDatabase::NAME_CLIENT_CACHE_ELEMENT && name != SystemDatabase::NAME_HIDE_ELEMENTS_ELEMENT && name != SystemDatabase::NAME_DEFAULT_RIGHT_ELEMENT) {
			PCubeArea path(new CubeArea(db, COMMITABLE_CAST(Cube, shared_from_this()), 1));
			PSet s(new Set);
			s->insert((*i)->getIdentifier());
			path->insert(0, s);
			Cube::setCellValue(server, db, path, CellValue(), PLockedCells(), user, boost::shared_ptr<PaloSession>(), false, false, DEFAULT, useJournal, 0, changedCubes, true, CubeArea::NONE);
		}
	}
	commitChanges(false, user, changedCubes, false);
}

void ConfigurationCube::clearCells(PServer server, PDatabase db, PCubeArea baseElements, PUser user, bool useJournal)
{
	throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "configuration cube cannot be cleared", "user", (int)user->getId());
}
...
void ConfigurationCube::checkAreaAccessRight(CPDatabase db, PUser user, CPCubeArea area, User::RightSetting& rs, bool isZero, RightsType minimumRight) const
{
	if (getMinimumAccessRight(user) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for configuration cube", "user", (int)user->getId());
	}
}

RightsType ConfigurationCube::getMinimumAccessRight(PUser user) const
{
	RightsType right = User::checkUser(user) ? user->getRoleRight(User::sysOpRight) : RIGHT_SPLASH;
	return right == RIGHT_NONE ? RIGHT_READ : right;
}

void ConfigurationCube::checkCubeAccessRight(PUser user, RightsType minimumRight) const
{
	if (getMinimumAccessRight(user) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for configuration cube", "user", (int)user->getId());
	}
...
RightsType ConfigurationCube::getElementAccessRight(CPDatabase db, PUser user, CPDimension dim, IdentifierType elemId, vector<User::RoleCubeRight>& rcRights) const
{
	return getMinimumAccessRight(user);
}
