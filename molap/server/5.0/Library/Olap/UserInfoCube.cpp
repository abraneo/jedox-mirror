...
{
	if (User::checkUser(user) && user->getRoleRight(User::userInfoRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for cube", "user", (int)user->getId());
	}
}

RightsType UserInfoCube::getMinimumAccessRight(PUser user) const
{
	return User::checkUser(user) ? user->getRoleRight(User::userInfoRight) : RIGHT_DELETE;
}

void UserInfoCube::checkCubeAccessRight(PUser user, RightsType minimumRight) const
{
	if (User::checkUser(user) && user->getRoleRight(User::userInfoRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for cube", "user", (int)user->getId());
	}
...
RightsType UserInfoCube::getElementAccessRight(CPDatabase db, PUser user, CPDimension dim, IdentifierType elemId, vector<User::RoleCubeRight>& rcRights) const
{
	return User::checkUser(user) ? user->getRoleRight(User::userInfoRight) : RIGHT_DELETE;
}
