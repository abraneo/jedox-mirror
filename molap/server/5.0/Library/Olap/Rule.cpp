...
// each restricted dim in source must also be restricted in destination
// e.g: {4@0} = B: {4@3} * {4@4}
// or {12@0,4@0} = B: {12@1,4@1} * ({12@2} / {4@3})
bool Rule::isSimpleRule(CPDatabase db, Variability &varDimensions) const
{
	return rule ? !hasMarkers() && checkSimpleRuleNode(db, rule->getExprNode(), varDimensions) : false;
}

bool Rule::checkSimpleRuleNode(CPDatabase db, Node* node, Variability &varDimensions) const
{
	switch(node->getNodeType())	{
		case Node::NODE_SOURCE_NODE: {
			SourceNode* src = dynamic_cast<SourceNode*>(node);

			if (src->canBeConsolidation(db)) {
				return false;
			}
			...
			vector<uint8_t>::iterator srcIt = restriction->begin();
			...
			return simpleRestrictions && src->isSimple();
#else
			// checking for simple restrictions (each restricted dim in source should also be restricted in destination)
			bool simpleRestrictions = true;

			vector<uint8_t> *restriction = src->getRestriction();
			vector<uint8_t>::iterator srcIt = restriction->begin();
			const Area *destArea = getDestinationArea();
			...
			for(size_t dimOrdinal = 0; srcIt != restriction->end(); ++srcIt, dimOrdinal++) {
				...
				// check if each restricted dim in source is also restricted in destination
				if(*srcIt == SourceNode::OFFSET_RESTRICTION || (*srcIt == SourceNode::ABS_RESTRICTION && !destArea->getDim(dimOrdinal))) {
					simpleRestrictions = false;
					break;
				}
			}
...
// TODO: -jj- enable with constant support
#ifdef GPU_RULES_CONST
		case Node::NODE_DOUBLE_NODE:
			return false; // empty variability
#endif

		case Node::NODE_FUNCTION_MULT: case Node::NODE_FUNCTION_DIV: {
			// recursive call for function operand nodes
			FunctionNodeSimple* fNode = dynamic_cast<FunctionNodeSimple*>(node);
			bool res1, res2, result;
			Variability leftVarDimensions;
			Variability rightVarDimensions;
			res1 = checkSimpleRuleNode(db, fNode->getLeftNode(), leftVarDimensions);
			res2 = checkSimpleRuleNode(db, fNode->getRightNode(), rightVarDimensions);
			if (res1 && res2) {
				result = true;
			} else if (!res1 && !res2) {
				result = false;
		...
		case Node::NODE_FUNCTION_ADD: case Node::NODE_FUNCTION_SUB: {
			// recursive call for function operand nodes
			FunctionNodeSimple* fNode = dynamic_cast<FunctionNodeSimple*>(node);
			Variability leftVarDimensions;
			Variability rightVarDimensions;
			bool result;
			result = checkSimpleRuleNode(db, fNode->getLeftNode(), leftVarDimensions);
			result &= checkSimpleRuleNode(db, fNode->getRightNode(), rightVarDimensions);
			varDimensions = leftVarDimensions;
			varDimensions |= rightVarDimensions;

			// both sources have to be variable
			if (leftVarDimensions.empty() || rightVarDimensions.empty()) {
				result = false;
			}
			return  result;
		}
...
static PRuleMarker convertMarker(CPCube cube, const Area* destination, Node* node)
{
	...
	if (Node::NODE_SOURCE_NODE == node->getNodeType()) {
		...
		PRuleMarker marker = PRuleMarker(new RuleMarker(make_pair(db->getId(), cube->getId()), area, destination));
		...
		PRuleMarker marker = PRuleMarker(new RuleMarker(make_pair(database->getId(), fromCube->getId()), make_pair(db->getId(), cube->getId()), path, destination));
		Logger::trace << "using " << *marker << endl;
