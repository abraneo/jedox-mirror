...
void MarkerStorage::generateMarkers(PCube fromCube, const Area* fromArea)
{
	...
	PSourcePlanNode sn(new SourcePlanNode(fromCube->getStringStorageId(), area, 0));
	PCellStream cs = engine->createProcessor(sn);
	while (cs && cs->next()) {
		addMarker(cs->getKey());
	}
	//go through numeric storage
	sn.reset(new SourcePlanNode(fromCube->getNumericStorageId(), area, 0));
	cs = engine->createProcessor(sn);
	while (cs->next()) {
		addMarker(cs->getKey());
	}
	//go through marker storage
	sn.reset(new SourcePlanNode(fromCube->getMarkerStorageId(), area, 0));
	cs = engine->createProcessor(sn);
	while (cs->next()) {
		addMarker(cs->getKey());
	}
	...
	cs = engine->createProcessor(cmpn);
	while (cs->next()) {
		addMarker(cs->getKey());
	}
}
