...
void LicenseInfoCube::clearCells(PServer server, PDatabase db, PUser user, bool useJournal)
{
}

void LicenseInfoCube::clearCells(PServer server, PDatabase db, PCubeArea baseElements, PUser user, bool useJournal)
{
	throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "license info cube cannot be cleared", "user", user ? (int)user->getId() : 0);
}

void LicenseInfoCube::checkPathAccessRight(PDatabase db, PUser user, PCubeArea cellPath, bool isZero, RightsType minimumRight) const
{
	if (User::checkUser(user) && user->getRoleRight(User::sysOpRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for license cube", "user", user ? (int)user->getId() : 0);
	}
}

RightsType LicenseInfoCube::getMinimumAccessRight(PUser user) const
{
	return User::checkUser(user) ? user->getRoleRight(User::sysOpRight) : RIGHT_READ;
}

void LicenseInfoCube::checkCubeAccessRight(PUser user, RightsType minimumRight) const
{
	if (User::checkUser(user) && user->getRoleRight(User::sysOpRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for license info cube", "user", user ? (int)user->getId() : 0);
	}
}

RightsType LicenseInfoCube::getElementAccessRight(CPDatabase db, PUser user, CPDimension dim, IdentifierType elemId, vector<User::RoleCubeRight>& rcRights) const
{
	return User::checkUser(user) ? user->getRoleRight(User::sysOpRight) : RIGHT_READ;
}
...
PCellStream LicenseInfoCube::evaluatePlan(PPlanNode plan, EngineBase::Type engineType) const
{
	return PCellStream(new LicenseInfoProcessor(plan->getArea(), Context::getContext()->getServer()->getLicenseIterator(false)));
}
