...
public:
	...
	void addDatabase(PDatabase, bool notify, bool newid);
	void removeDatabase(PDatabase, bool notify);
	void renameDatabase(PDatabase, const string & name, bool notify);
	PDatabase addDatabase(const string & name, PUser user, IdentifierType type = 0);
	void deleteDatabase(PDatabase database, PUser user);
	void renameDatabase(PDatabase database, const string & name, PUser user);
...
public:
	...
	virtual void updateGpuBins(PDimension dimension, PServer server, PaloJobRequest* jobRequest, PDatabase database, Context* context, PUser user);
