...
Commitable::Commitable(const string &n) :
		copiesCount(0), version(0), comm_id(-1), checkedOut(true), comm_name(n)
{
}

Commitable::Commitable(const Commitable &c) :
		old(c.shared_from_this()), copiesCount(0), version(c.version + 1000000000), comm_id(c.comm_id), checkedOut(true), comm_name(c.comm_name)
{
...
void Commitable::mergeint(const CPCommitable &o)
{
	if (old != 0) {
	...
	if (o) {
#ifdef USE_BOOST_ATOMIC
		BOOST_INTERLOCKED_INCREMENT( &(const_cast<Commitable *>(o.get())->version) );
#else
		version = const_cast<Commitable *>(o.get())->version.fetch_add(1, std::memory_order_relaxed);
#endif
	}
}
