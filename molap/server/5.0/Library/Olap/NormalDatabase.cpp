...
void NormalDatabase::notifyAddDatabase(PServer server)
{
	if (status != UNLOADED) {
		// in write mode
		bool dbChanged;
		addSystemDimension(server, dbChanged);
	}
}
...
void NormalDatabase::addSystemDimension(PServer server, bool &dbChanged)
{
	checkCheckedOut();
