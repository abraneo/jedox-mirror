...
PaloSession::PaloSession(string sid, IdentifierType id, PUser user, bool worker, time_t ttlIntervall, const string peerName, const string description, const string &machineId) :
	sid(sid), sidId(id), worker(worker), timeToLiveIntervall(ttlIntervall), workercontext(0), requestCounter(0), totalTime(0), peerName(peerName), description(description), machineId(machineId)
{
...
boost::shared_ptr<PaloSession> PaloSession::createSession(string sid, IdentifierType sidId, PUser user, bool worker, time_t ttlIntervall, bool testSession, const string peerName, const string *machine, const string *required, string *optional, const string description)
{
	boost::shared_ptr<PaloSession> session(new PaloSession(sid, sidId, user, worker, ttlIntervall, peerName, description, machine ? *machine : ""));
	if (!testSession) {
...
boost::shared_ptr<PaloSession> PaloSession::createSession(PUser user, bool worker, time_t ttlIntervall, bool shortSid, bool testSession, const string peerName, const string *machine, const string *required, string *optional, const string description)
{
...

	{
		...
		result = createSession(sidString, testSession ? 0 : ++lastSessionId, user, worker, ttlIntervall, testSession, peerName, machine, required, optional, description);
	}
	return result;
}
...
PUser PaloSession::getUser() const
{
	...
	if (user == 0) {
		if (sd) {
			Logger::warning << "user in session not found, sd " << (sd ? "exists" : "is null") << ", userIdentifier: " << userIdentifier << ", useExternalUser: " << sd->getUseExternalUser() << endl;
		} else {
			Logger::warning << "user in session not found, sd " << (sd ? "exists" : "is null") << ", userIdentifier: " << userIdentifier << endl;
		}
...
void PaloSession::increaseTime(uint64_t microseconds, const PaloJob *job) {
	if (Server::flightRecorderEnabled()) {
		...
		if (isWorker()) {
			finishedJobs.push_back(JOB_INFO("SVS-worker", job->getStartTime(), job->getDuration(), job->getRequest()));
		} else {
			PUser user;
			try {
				user = getUser();
			} catch (...) {
				Logger::warning << "user not found exception caught in increaseTime" << endl;
			}
			finishedJobs.push_back(JOB_INFO(user ? user->getName() : "FAKE", job->getStartTime(), job->getDuration(), job->getRequest()));
		}
...
{
	return Context::getContext()->getServer();
...
void PaloSession::requestRecord(string req, string ext)
{
	...
	if (recorderFile) {
		...
		*recorderFile << "#USER" << '\t' << "START" << '\t' << "DURATION" << '\t' << "REQUEST" << endl;
		...
		if (isWorker()) {
			*stream << "SVS-worker" << '\t';
			...
			try {
				...
				Logger::error << "user not found exception caught in writeActiveJobsRecords" << endl;
			}
			*stream << (user ? user->getName() : "FAKE") << '\t';
		}
