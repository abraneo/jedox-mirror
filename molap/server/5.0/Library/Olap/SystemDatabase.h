...
public:
	...
	SystemDatabase(const string& name) :
		...
		groupPropertiesDimensionId(NO_IDENTIFIER), roleDimensionId(NO_IDENTIFIER), rightObjectDimensionId(NO_IDENTIFIER),
		...
		roleRolePropertiesCubeId(NO_IDENTIFIER), groupGroupPropertiesCubeId(NO_IDENTIFIER),
...
public:
	...
	void createSystemItems(PServer server, bool forceCreate = false);
	...
	PUser createExternalUser(PServer server, const string&, vector<string>*);
...
private:
	PDimension checkAndCreateDimension(PServer server, const string&, const ElementItem* elemBegin = 0,  const ElementItem* elemEnd = 0, bool rebuild = false);
