...
public:
	...
	vector<CPCube> getCubes(PUser userP, PDatabase db, const ItemType *filterType = 0) const;
	...
	virtual ElementsType getElements(PUser user, bool onlyRoot) const;
	...
	virtual Element *findElementByPosition(PositionType position, User *user, bool write) const;
	...
	void removeElementsFromCubes(PServer server, PDatabase db, PUser user, IdentifiersType elementIds, bool deleteRules, CubeRulesArray* disabledRules, DeleteCellType delType, bool completeRemove);
...
protected:
	...
	virtual void checkCubeAccessRight(PUser user, RightsType minimumRight) const {
		if (User::checkUser(user)) user->checkRoleRight(User::cubeRight, minimumRight);
	}
	...
	virtual void checkElementAccessRight(User *user, RightsType minimumRight) const {
		if (User::checkUser(user)) user->checkRoleRight(User::elementRight, minimumRight);
	}
...
protected:
	virtual bool hasStringAttributes() {
		return false;
	}
...
private:
	...
	void removeElementFromCubes(PServer server, PDatabase db, PUser user, IdentifierType elementId, bool deleteRules, CubeRulesArray* disabledRules, DeleteCellType delType, bool completeRemove);
