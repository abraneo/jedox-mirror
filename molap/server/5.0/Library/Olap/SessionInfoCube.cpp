...
class SERVER_CLASS SessionInfoProcessor : public CellValueStream {
public:
	SessionInfoProcessor(CPArea filter) : filter(filter), paloSessionLock(&PaloSession::m_main_Lock), context(Context::getContext()) {}
...
void SessionInfoCube::clearCells(PServer server, PDatabase db, PUser user, bool useJournal)
{
}

void SessionInfoCube::clearCells(PServer server, PDatabase db, PCubeArea baseElements, PUser user, bool useJournal)
{
	throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "session info cube cannot be cleared", "user", user ? (int)user->getId() : 0);
}

void SessionInfoCube::checkPathAccessRight(PDatabase db, PUser user, PCubeArea cellPath, bool isZero, RightsType minimumRight) const
{
	if (User::checkUser(user) && user->getRoleRight(User::sysOpRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for configuration cube", "user", user ? (int)user->getId() : 0);
	}
}

RightsType SessionInfoCube::getMinimumAccessRight(PUser user) const
{
	return User::checkUser(user) ? user->getRoleRight(User::sysOpRight) : RIGHT_READ;
}

void SessionInfoCube::checkCubeAccessRight(PUser user, RightsType minimumRight) const
{
	if (User::checkUser(user) && user->getRoleRight(User::sysOpRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for session info cube", "user", user ? (int)user->getId() : 0);
	}
}

RightsType SessionInfoCube::getElementAccessRight(CPDatabase db, PUser user, CPDimension dim, IdentifierType elemId, vector<User::RoleCubeRight>& rcRights) const
{
	return User::checkUser(user) ? user->getRoleRight(User::sysOpRight) : RIGHT_READ;
}
...
PCellStream SessionInfoCube::evaluatePlan(PPlanNode plan, EngineBase::Type engineType) const
{
	PCellStream result(new SessionInfoProcessor(plan->getArea()));
	return result;
}
