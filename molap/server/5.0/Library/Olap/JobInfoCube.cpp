...
class SERVER_CLASS JobInfoProcessor : public CellValueStream {
public:
	JobInfoProcessor(CPArea filter) : filter(filter), paloJobLock(&PaloJob::m_main_Lock) {}
...
void JobInfoCube::clearCells(PServer server, PDatabase db, PUser user, bool useJournal)
{
}

void JobInfoCube::clearCells(PServer server, PDatabase db, PCubeArea baseElements, PUser user, bool useJournal)
{
	throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "job info cube cannot be cleared", "user", user ? (int)user->getId() : 0);
}

void JobInfoCube::checkPathAccessRight(PDatabase db, PUser user, PCubeArea cellPath, bool isZero, RightsType minimumRight) const
{
	if (User::checkUser(user) && user->getRoleRight(User::sysOpRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for jobinfo cube", "user", user ? (int)user->getId() : 0);
	}
}

RightsType JobInfoCube::getMinimumAccessRight(PUser user) const
{
	return User::checkUser(user) ? user->getRoleRight(User::sysOpRight) : RIGHT_READ;
}

void JobInfoCube::checkCubeAccessRight(PUser user, RightsType minimumRight) const
{
	if (User::checkUser(user) && user->getRoleRight(User::sysOpRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for job info cube", "user", user ? (int)user->getId() : 0);
	}
}

RightsType JobInfoCube::getElementAccessRight(CPDatabase db, PUser user, CPDimension dim, IdentifierType elemId, vector<User::RoleCubeRight>& rcRights) const
{
	return User::checkUser(user) ? user->getRoleRight(User::sysOpRight) : RIGHT_READ;
}
...
PCellStream JobInfoCube::evaluatePlan(PPlanNode plan, EngineBase::Type engineType) const
{
	PCellStream result(new JobInfoProcessor(plan->getArea()));
	return result;
}
