...
bool Lock::overlaps(CPArea area) const
{
	...
	Area::ConstDimIter dimIterA = area->dimBegin();
	Area::ConstDimIter dimIterB = overlapArea->dimBegin();

	for (; dimIterA != area->dimEnd(); ++dimIterA, ++dimIterB) {
		CPSet identifiersA = (*dimIterA);
		CPSet identifiersB = (*dimIterB);
