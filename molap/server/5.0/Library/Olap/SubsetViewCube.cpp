...
void SubsetViewCube::checkAreaAccessRight(CPDatabase db, PUser user, CPCubeArea area, User::RightSetting& rs, bool isZero, RightsType minimumRight) const
{
	...
	if (User::checkUser(user) && user->getRoleRight(User::subSetViewRight) < minimumRight) {
...
RightsType SubsetViewCube::getMinimumAccessRight(PUser user) const
{
	return User::checkUser(user) ? user->getRoleRight(User::subSetViewRight) : RIGHT_SPLASH;
}

void SubsetViewCube::checkCubeAccessRight(PUser user, RightsType minimumRight) const
{
	if (User::checkUser(user) && user->getRoleRight(User::subSetViewRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for configuration cube", "user", (int)user->getId());
	}
...
RightsType SubsetViewCube::getElementAccessRight(CPDatabase db, PUser user, CPDimension dim, IdentifierType elemId, vector<User::RoleCubeRight>& rcRights) const
{
	return User::checkUser(user) ? user->getRoleRight(User::subSetViewRight) : RIGHT_SPLASH;
}
