...
#include "Engine/StorageCpu.h"
	...
	#include "PaloGpu/GpuStorage/GpuStorage.h"
#endif
...
double Cube::cacheClearBarrierCells = 10000.0;
...
Cube::Cube(PDatabase db, const string& name, const IdentifiersType *dimensions, Cube::SaveType saveType) :
	...
	cache(dimensions->size(), cacheBarrier, cacheClearBarrierCells), additiveCommit(false), delCount(0)
{
	if (!token) {
		++token;
	}
...
Cube::Cube(const Cube& c) :
	...
	wholeCubeLocked(c.wholeCubeLocked), pathTranslator(c.pathTranslator), cache(c.dimensions.size(), cacheBarrier, cacheClearBarrierCells),
...
bool Cube::loadCubeCells(FileReader *fr, PDatabase db, bool loadGroups, bool binary, bool &diffGroup, uint32_t fileVersion)
{
	checkCheckedOut();
	...
	if (loadGroups) {
		...
		PDimension groupDimension = db->lookupDimensionByName(SystemDatabase::NAME_GROUP_DIMENSION, false);

		if (fr->isSectionLine() && fr->getSection() == "GROUP") {
			fr->nextLine();

			while (fr->isDataLine()) {
				IdentifierType id = fr->getDataInteger(0);
				string name = fr->getDataString(1);
				Element * groupElement = groupDimension->lookupElementByName(name, false);

				if (binary) {
					if (!groupElement || id != groupElement->getIdentifier()) {
						ret = false; //different group element -> load csv file due to mapping
						diffGroup = true;
						break;
					}
				} else {
			...
			ElementsType elements = groupDimension->getElements(PUser(), false);
	...
	if (ret) {
		...
		if (binary) {
			StorageCpu *st;
			if (!loadGroups) {
				if (fr->isSectionLine() && fr->getSection() == "NUMERIC") {
					...
					if (getType() == GPUTYPE) {
						...
						numericGpuStorage->setCellValue(st->getCellValues(CPArea(), 0));
					}
				} else {
					ret = false;
				}
			}
			if (ret && fr->isSectionLine() && fr->getSection() == "STRING") {
			...
			bool numFound = groupMap ? true : false; //group cubes doesn't have numeric section
			bool strFound = false;
			do {
				if (fr->isSectionLine()) {
					if (fr->getSection() == "NUMERIC") {
						numFound = true;
						...
						numericStorage->setCellValue(PCellStream(new CubeFileStream(db, COMMITABLE_CAST(Cube, shared_from_this()), fr, false, groupMap)));

						if (numericGpuStorage) {
							// synchronize CPU storage with GPU
							numericCpuStorage->setCellValue(numericGpuStorage->getCellValues(CPArea(), 0));
						}
		...
		//				}
						...
						// load string storage
						stringStorage->setCellValue(PCellStream(new CubeFileStream(db, COMMITABLE_CAST(Cube, shared_from_this()), fr, true, groupMap)));
		...
		//				}
			...
			if (!numFound || !strFound) {
				if (!numFound) {
					throw FileFormatException("section 'NUMERIC' not found", fr);
				} else {
					throw FileFormatException("section 'STRING' not found", fr);
				}
			}
		}
	}
...
void Cube::processJournalCommand(PServer server, PDatabase db, CPCube thisCube, JournalFileReader &history, InBulkEnum &replaceBulkState, set<PCube> &changedCubes)
{
	...
	if (command == JournalFileReader::JOURNAL_VERSION) {
		...
		createRule(server, db, ruleNode, definition, external, comment, activate, PUser(), false, &id);
		...
		modifyRule(server, db, rule, ruleNode, definition, external, comment, PUser(), activate, false);
...
boost::shared_ptr<JournalFileReader> Cube::initJournalProcess()
{
	...
	if (fileName) {
		...
		if (!journalReader->openFile(false)) {
			journalReader.reset();
		}
	}
...
void Cube::loadCube(PServer server, PDatabase db, bool processJournal)
{
	...
	loadCubeIntern(server, db, processJournal, true, false);
}

void Cube::loadCubeIntern(PServer server, PDatabase db, bool processJournal, bool checkIgnore, bool checkGroup)
{
	checkCheckedOut();
	if (fileName == 0) {
...

	{
		...
		if (checkGroup) {
			PDimension groupDimension = db->lookupDimensionByName(SystemDatabase::NAME_GROUP_DIMENSION, false);
			loadGroups = !dimensions.empty() && dimensions[0] == groupDimension->getId();
		}
		loadFromFile(server, db, loadCells, loadGroups, loadBin);
...
void Cube::loadFromFile(PServer server, PDatabase db, bool loadCells, bool loadGroups, bool binary)
{
	...
	if (binary) {
		...
		if (!err && loadCells) {
			if (fr->isSectionLine()) {
				bool diffGroup;
				if (!loadCubeCells(fr.get(), db, loadGroups, true, diffGroup, ver)) {
					if (diffGroup) {
						Logger::info << "group dimension changed, loading cube '" << getName() << "' from csv file" << endl;
					} else {
	...
	if (!binary || err) {
		...
		if (loadCells) {
			if (fr->isSectionLine()) {
				bool diffGroup;
				loadCubeCells(fr.get(), db, loadGroups, false, diffGroup, 0);
			} else {
...
void Cube::saveCubeCells(FileWriter *file, PServer server, PDatabase db, bool saveGroups, bool binary)
{
	CPCube thisCube = CONST_COMMITABLE_CAST(Cube, shared_from_this());
	PCubeArea area(new CubeArea(db, thisCube, dimensions.size()));
	PSet groupSet(new Set);
	...
	if (saveGroups) {
		...
		for (ElementsType::iterator iter = elements.begin(); iter != elements.end(); ++iter) {
			...
			if (element != 0) {
				...
				file->appendEscapeString(element->getName(groupDimension->getElemNamesVector()));
				file->nextLine();
			}
		}
	}
	...
	for (size_t i = 0; i < dimensions.size(); i++) {
		if (saveGroups && i == 0) {
			area->insert(0, groupSet);
		} else {
			PDimension dim = db->lookupDimension(dimensions[i], false);
			PElementList elems = dim->getElementList();
			PSet s(new Set);
			bool isAlias = false;
			...
			if (!isAlias) {
				for (ElementList::const_iterator it = elems->begin(); it != elems->end(); ++it) {
					if ((*it).getElementType() != Element::UNDEFINED) {
						s->insert((*it).getIdentifier());
					}
				}
			}
...
{
	saveCubeIntern(server, db, true, false);
}

void Cube::saveCubeIntern(PServer server, PDatabase db, bool checkIgnore, bool checkGroup)
{
	if (fileName == 0) {
		throw ErrorException(ErrorException::ERROR_INTERNAL, "cube file name not set");
	}
	...
	if (cellsStatus == CHANGED) {
		...
		if (checkGroup) {
			PDimension groupDimension = db->lookupDimensionByName(SystemDatabase::NAME_GROUP_DIMENSION, false);
			saveGroups = !dimensions.empty() && dimensions[0] == groupDimension->getId();
		}
		...
		timeval tv;
		gettimeofday(&tv, 0);
		saveToFile(server, db, saveCells, saveGroups, tv, true);
		if (hasBin || !hasCsv) { //don't save CSV if the cube was loaded from it
			saveToFile(server, db, saveCells, saveGroups, tv, false);
		}
	}
...
void Cube::saveToFile(PServer server, PDatabase db, bool saveCells, bool saveGroups, timeval &tv, bool binary)
{
	...
	if (saveCells) {
		saveCubeCells(fw.get(), server, db, saveGroups, binary);
	}
...
vector<PRule> Cube::getRules(PUser user) const
{
	...
	for (RuleList::Iterator iter = rules->begin(); iter != rules->end(); ++iter) {
		if (*iter != 0) {
			result.push_back(COMMITABLE_CAST(Rule, *iter));
			if (context) {
				context->saveParent(shared_from_this(), *iter);
			}
		}
	}
...
void Cube::clearCells(PServer server, PDatabase db, PUser user, bool useJournal)
{
	...
	if (saveType == RIGHTS) {
		if (getName() == SystemDatabase::NAME_USER_GROUP_CUBE || getName() == SystemDatabase::NAME_GROUP_ROLE || getName() == SystemDatabase::NAME_ROLE_RIGHT_OBJECT_CUBE) {
			throw ParameterException(ErrorException::ERROR_CUBE_UNDELETABLE, "cube cannot be deleted", "cube", getName());
		}
	}
	...
	checkCubeAccessRight(user, getType() == USER_INFOTYPE ? RIGHT_WRITE : RIGHT_DELETE);
...
void Cube::clearCells(PServer server, PDatabase db, PCubeArea areaElements, PUser user, bool useJournal)
{
	...
	if (User::checkUser(user)) {
		...
		if (saveType == NORMAL) {
			User::MinMaxRight rtRCD = user->getRCDDataRight(db, getId());

			bool enough = rtRCD.second >= minimumRight;
			if (enough) {
				bool checkRole = rtRCD.first >= minimumRight ? false : user->getRoleCellDataRight().first < minimumRight;
				bool checkCube = rtRCD.first >= minimumRight ? false : user->getCubeDataRight(db, getId()).first < minimumRight;
				bool checkDims = rtRCD.first < minimumRight;
				bool checkCells = user->checkCellDataRightCube(db, cube);

				if (checkRole || checkCube || checkDims || checkCells) {
					...
					if (enough && (checkDims || checkCells)) {
						...
						enough = user->checkDimsAndCells(db, cube, userGroups, rootArea, checkDims, checkCells, minimumRight);
					}
				}
			}
			if (!enough) {
	...
	for (SubCubeList::iterator it = cons.begin(); it != cons.end(); ++it) {
		PCubeArea baseArea = it->second->expandBase(0);
...
void Cube::copyCellCreateMaps(PDatabase db, Element *sourceElem, Element *targetElem, CPDimension dim, PSetMultimap &setMultimap,
		IdentifierType &targetTransformId, Set &sourceSet, Set &targetSet, set<IdentifierType> &targetForbiddenElems, Id2IdMap &target2SourceMap, bool &simpleCase)
{
	...
	if (sourceElem->getElementType() == Element::CONSOLIDATED && targetElem->getElementType() == Element::CONSOLIDATED) {
		...
		if (sourceChildrenCount == targetChildrenCount) {
			for (size_t i = 0; i < sourceChildrenCount; i++) {
				...
				copyCellCreateMaps(db, (childrenSource)[i].first, (childrenTarget)[i].first, dim, setMultimap, targetTransformId, sourceSet, targetSet, targetForbiddenElems, target2SourceMap, simpleCase);
			}
			return; // all done
		}
	}
...
bool Cube::copyCellValuesPrepare(PServer server, PDatabase db, PCubeArea cellPathFrom, PCubeArea cellPathTo, PLockedCells lockedCells, PUser user, CubeArea &areaSource,
		CubeArea &areaTarget, SetMultimaps &setMultimaps, IdentifiersType &transformationMask, double &factorToReturn, double *dValue, double *factorFromJournal, bool &simpleCase)
{
	...
	if (dValue && *dValue == 0.0) {
		throw ParameterException(ErrorException::ERROR_INVALID_COPY_VALUE, "wrong value for copy like", "value", 0.0);
	}

	factorToReturn = 1.0;
	CellValue cellValue = getCellValue(cellPathFrom);
	...
	if (!cellValue.isEmpty()) {
		...
		for (uint32_t i = 0; i < dimCount; i++) {
			...
			if (sourceElement == targetElement) {
				...
				copyCellCreateMaps(db, sourceElement, targetElement, dim, setMultimaps[i], transformationMask[i], *sourceSet, *targetSet, targetForbiddenElems[i], target2SourceMap, simpleCase);
		...
		set<PCube> changedCubes;
		setCellValue(server, db, cellPathTo, cellValue, lockedCells, PUser(), boost::shared_ptr<PaloSession>(), false, false, DEFAULT, true, 0, changedCubes, false, CubeArea::NONE);
...
//	CubeArea &areaTarget, SetMultimaps &setMultimaps

	bool simpleCase = true;
	if (!copyCellValuesPrepare(server, db, cellPathFrom, cellPathTo, lockedCells, user, *areaSrc, *areaTarget, setMultimaps, transformationMask, factor, dValue, factorFromJournal, simpleCase)) {
		// nothing prepared to be computed, all done
		return true;
	}

	PPlanNode sourcePlan = createPlan(areaSrc, CubeArea::NUMERIC, bUseRules ? ALL_RULES : NO_RULES, true, UNLIMITED_UNSORTED_PLAN);
	if (User::checkUser(user)) {
	...
	PPlanNode targetPlan = PPlanNode(new TransformationPlanNode(areaTarget, sourcePlan, transformationMask, setMultimaps, factor));
	...
	if (simpleCase) { // Todo: -jj- additional cases has to be eliminated (source or target is consolidation, multidimensional rights cube)
		...
		if (delValue) {
			...
			if (!wholeCubeLocked && !fromMarkers.empty()) {
				...
				PCellStream targetData = engine->createProcessor(targetPlan);
...
//		Logger::info << "Target Plan: " << targetPlan->toXML() << endl;

		PCellStream csMappedSource = evaluatePlan(targetPlan, EngineBase::ANY);
...
ResultStatus Cube::setCellValue(PServer server, PDatabase db, PCubeArea spCellPath, CellValue value, PLockedCells lockedCells, PUser user, boost::shared_ptr<PaloSession> session, bool checkArea, bool addValue, SplashMode splashMode, bool bWriteToJournal, User::RightSetting* checkRights, set<PCube> &changedCubes, bool possibleCommit, CubeArea::CellType ct)
{
	...
	if (cellType == CubeArea::CONSOLIDATED) {
		...
		cellPlan = createPlan(spCellPath, CubeArea::ALL, NO_RULES, false, UNLIMITED_UNSORTED_PLAN);
	...
	if (isAggregation) {
		...
		if (splashMode == DEFAULT) {
			PCellStream cs = evaluatePlan(cellPlan, EngineBase::ANY);
			cs->next();
			...
			if (anyActiveRestriction) {
				...
				cs = evaluatePlan(cellPlan, EngineBase::ANY); // Todo: -jj-test if GPU engine can aggregate on reduced base areas
				cs->next();
...
ResultStatus Cube::setCellValue(bool isBulk)
{
	...
	if (disjunctive) { // splash mode is DEFAULT
...
//			Logger::debug << "collect - start" << endl;
			size_t i;
			for (i = pos; i < cvc->pathVectorAndValue.size(); i++) {
				...
				if (pvv.cellType != CubeArea::CONSOLIDATED) { // CubeArea::BASE_NUMERIC || CubeArea::BASE_STRING
					...
					if (i == pos) {
						...
						if (!cvc->addValue && zeroGroup) {
							break;
						}
						bool isZero = pvv.value.isEmpty();
						if (!cvc->addValue && isZero) {
							break;
						}
...
//			Logger::debug << "setCellValue - start" << endl;
//			cout << "values from <" << pos << ", " << i << ") are prepared, multicount = " << multicount;
...
//			cout << ", contcount = " << contcount << ", zerocount = " << zerocount << endl;
//			Logger::debug << "setCellValue - end" << endl;
...
CellValue Cube::getCellValue(PCubeArea cellPath) const
{
	PCellStream cs = calculateArea(cellPath, CubeArea::ALL, ALL_RULES, false, UNLIMITED_UNSORTED_PLAN);
	cs->next();
	return cs->getValue();
}
...
PPlanNode Cube::createPlan(PCubeArea area, CubeArea::CellType type, RulesType paramRulesType, bool skipEmpty, uint64_t blockSize) const
{
	...
	if (pn && Logger::isTrace() /*&& pn->getChildren().size()*/) {
		Logger::trace << "Plan: " << pn->toXML() << endl;
	}
	return pn;
}

PCellStream Cube::evaluatePlan(PPlanNode plan, EngineBase::Type engineType) const
{
	Context *context = Context::getContext();
...
//        }
	PEngineBase engine;
	if (useGpu && plan->getType() == AGGREGATION) {
		const AggregationPlanNode *apn = dynamic_cast<const AggregationPlanNode *>(plan.get());
		if (apn->getAggregationType() != AggregationPlanNode::SUM) {
			useGpu = false;
		} else if (plan->getArea()->getSize() <= 1000) {
			// condition are Ok
	...
	PCellStream result = engine->createProcessor(plan);
	return result;
}
...
PCellStream Cube::calculateArea(PCubeArea area, CubeArea::CellType type, RulesType paramRulesType, bool skipEmpty, uint64_t blockSize) const
{
	...
	PCellStream result;
	if (plan) {
		result = evaluatePlan(plan, EngineBase::ANY);
	}
	return result;
}

void Cube::invalidateCache()
{
	if (cache.clear()) {
...
void Cube::cellGoalSeekEqualRelative(PServer server, PDatabase db, PCubeArea cellPath, PUser user, boost::shared_ptr<PaloSession> session, double value, vector<IdentifiersWeightMap> &siblings, bool equal, User::RightSetting& rs)
{
	...
	PCellStream origValue = calculateArea(cellPath, CubeArea::NUMERIC, ALL_RULES, true, UNLIMITED_UNSORTED_PLAN);
	origValue->next();
	...
	for (size_t i = 0; i < (*origPath).size(); i++) {
		...
		PCellStream cs = evaluatePlan(plan, EngineBase::ANY);
		if (!equal) {
			while (cs->next()) {
			...
			cs = evaluatePlan(plan, EngineBase::ANY);
//			cs->reset();
		}
		if (sum == 0) {
...
PRule Cube::createRule(PServer server, PDatabase db, PRuleNode node, const string& definition, const string& external, const string& comment, bool activate, PUser user, bool useJournal, IdentifierType *id)
{
	checkCheckedOut();
...
bool Cube::modifyRule(PServer server, PDatabase db, PRule rule, PRuleNode node, const string& definition, const string& external, const string& comment, PUser user, ActivationType activation, bool useJournal)
{
	checkCheckedOut();
	if (getStatus() == UNLOADED && useJournal) {
		throw ParameterException(ErrorException::ERROR_CUBE_NOT_LOADED, "cube not loaded", "cube", getId());
...
bool Cube::activateRules(PServer server, PDatabase db, const vector<PRule> &rules, ActivationType activation, PUser user, string* errMsg, bool bDefinitionChangedBefore, bool useJournal)
{
	...
	for (vector<PRule>::const_iterator ruleIt = rules.begin(); ruleIt != rules.end(); ++ruleIt) {
		...
		if (!rule->setActive(newstate, server, db, COMMITABLE_CAST(Cube, shared_from_this()), errMsg)) {
			return false;
		}
...
bool Cube::deleteElement(PServer server, PDatabase db, PUser user, const string& event, CPDimension dimension, IdentifierType element, bool deleteRules, CubeRulesArray *disabledRules, Dimension::DeleteCellType delType, bool completeRemove)
{
	PSet fullSet(new Set(true));
	return deleteElements(server, db, user, event, dimension, IdentifiersType(1, element), deleteRules, disabledRules, delType, fullSet, completeRemove);
}

bool Cube::deleteElements(PServer server, PDatabase db, PUser user, const string& event, CPDimension dimension, IdentifiersType elements, bool deleteRules, CubeRulesArray* disabledRules, Dimension::DeleteCellType delType, PSet fullSet, bool completeRemove)
{
	checkCheckedOut();
	size_t dim = (size_t) - 1;
	...
	if (dimFound) {
		...
		CPCube cube = CONST_COMMITABLE_CAST(Cube, shared_from_this());
		PCubeArea baseArea(new CubeArea(db, cube, dimCount));
		PCubeArea allArea(new CubeArea(db, cube, dimCount));
		PCubeArea numericArea(new CubeArea(db, cube, dimCount));
		if (getStatus() != UNLOADED || Context::getContext()->getInJournal()) {
			PSet s(new Set);
			for (size_t i = 0; i < elements.size(); i++) {
				IdentifierType elemId = elements[i];

				if (elemId == (IdentifierType) - 1) {
					Logger::warning << "element Id is -1" << endl;
					continue;
				}
				s->insert(elemId); //insert an element for expandStar
				break;
			}
			if (!s->size()) {
				return false;
			}
			baseArea->insert(dim, s);
			allArea->insert(dim, s);
			numericArea->insert(dim, s);
		}
		...
		PSet numElemsToDelete = PSet(new Set()); // optimization, delete in bulk
		...
		for (size_t i = 0; i < elements.size(); i++) {
			...
			bool bDeleteNumLater = false;

			if (getStatus() != UNLOADED || Context::getContext()->getInJournal()) {
				PCubeArea area;

				Element* element = dimension->findElement(elemId, 0, false);
				bool isString = Dimension::isStringElement(element->getElementType(), element->isStringConsolidation());

				if (delType == Dimension::DEL_ALL) { // allArea will be used
					if (!isString) { // numbers will be deleted later in bulk
						bDeleteNumLater = true;
						numElemsToDelete->insert(elemId);
					}
					if (!allArea->getSize()) { // not expanded yet, expand for strings
						allArea.reset(new CubeArea(db, cube, *allArea->expandStar(CubeArea::ALL_ELEMENTS)));
					}
					area = allArea;
				} else if (delType == Dimension::DEL_NUM) {
					if (!isString) {
						bDeleteNumLater = true;
						numElemsToDelete->insert(elemId);
					} else {
						// nothing will be done (string element has no numeric values)
					}
					area = baseArea;
				} else if (delType == Dimension::DEL_STR) {
					if (completeRemove) {
						if (!allArea->getSize()) { // not expanded yet
							allArea.reset(new CubeArea(db, cube, *allArea->expandStar(CubeArea::ALL_ELEMENTS)));
						}
						area = allArea;
					} else {
						if (!numericArea->getSize()) { // not expanded yet
							numericArea.reset(new CubeArea(db, cube, *numericArea->expandStar(CubeArea::NUMERIC_ELEMENTS)));
						}
						area = numericArea;
					}
				}
				...
				if (area->getSize()) {
					...
					if (!bDeleteNumLater && (delType & Dimension::DEL_NUM) && !isString) {
		...
		// delete in bulk
		if (numElemsToDelete->size() > 0) {
			baseArea.reset(new CubeArea(db, cube, *baseArea->expandStarOptim(fullSet)));
			if (baseArea->getSize()) {
				baseArea->insert(dim, numElemsToDelete);
...
void Cube::checkAreaAccessRight(CPDatabase db, PUser user, CPCubeArea area, User::RightSetting& rs, bool isZero, RightsType minimumRight) const
{
	if (User::checkUser(user)) {
		CPCube cube = CONST_COMMITABLE_CAST(Cube, shared_from_this());
		user->checkAreaRightsComplete(db, cube, area, rs, isZero, minimumRight);
	}
}

//returns false if minimal right is RIGHT_NONE, otherwise true
RightsType Cube::getMinimumAccessRight(PUser user) const
{
	RightsType ret = RIGHT_SPLASH;
	if (User::checkUser(user)) {
		CPDatabase db = CONST_COMMITABLE_CAST(Database, Context::getContext()->getParent(shared_from_this()));
		ret = user->getRCDDataRight(db, getId()).first;
	}
	return ret;
}

void Cube::checkCubeAccessRight(PUser user, RightsType minimumRight) const
{
	if (User::checkUser(user)) {
		Context* context = Context::getContext();
		CPCube cube = CONST_COMMITABLE_CAST(Cube, shared_from_this());
		...
		User::MinMaxRight rtCube = user->getCubeDataRight(db, getId());

		bool enough = rtRole.second >= minimumRight && rtCube.second >= minimumRight;
		if (enough) {
			bool checkRole = rtRole.first < minimumRight;
			bool checkCube = rtCube.first < minimumRight;
			if (checkRole || checkCube) {
				set<IdentifierType> userGroups;
				user->getUserGroupsCopy(userGroups);
...
RightsType Cube::getElementAccessRight(CPDatabase db, PUser user, CPDimension dim, IdentifierType elemId, vector<User::RoleCubeRight>& rcRights) const
{
	return User::checkUser(user) ? user->getDimensionDataRight(db, dim, elemId, rcRights) : RIGHT_SPLASH;
}
...
void Cube::checkCubeRuleRight(PUser user, RightsType minimumRight) const
{
	if (User::checkUser(user)) {
		...
		User::MinMaxRight rtCube = user->getCubeDataRight(db, getId());
		bool enough = rtCube.second >= RIGHT_READ;
		if (enough) {
			set<IdentifierType> userGroups;
			user->getUserGroupsCopy(userGroups);
			...
			if (enough && rtCube.first < RIGHT_READ) {
				enough = user->checkCubeDataRight(db, cube, userGroups, RIGHT_READ);
			}
		}
		if (!enough) {
			throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights", "user", (int)user->getId());
