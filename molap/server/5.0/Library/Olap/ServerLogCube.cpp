...
class SERVER_CLASS ServerLogProcessor : public CellValueStream {
public:
	ServerLogProcessor(CPArea filter) : filter(filter), paloLog(0), counter(NO_IDENTIFIER), pathIt(filter->pathBegin()) {}
...
void ServerLogCube::clearCells(PServer server, PDatabase db, PUser user, bool useJournal)
{
	throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "server log cube cannot be cleared", "user", (int)user->getId());
}

void ServerLogCube::clearCells(PServer server, PDatabase db, PCubeArea baseElements, PUser user, bool useJournal)
{
	throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "server log cube cannot be cleared", "user", (int)user->getId());
}

void ServerLogCube::checkPathAccessRight(PDatabase db, PUser user, PCubeArea cellPath, bool isZero, RightsType minimumRight) const
{
...
RightsType ServerLogCube::getMinimumAccessRight(PUser user) const
{
	return User::checkUser(user) ? user->getRoleRight(User::sysOpRight) : RIGHT_SPLASH;
}

void ServerLogCube::checkCubeAccessRight(PUser user, RightsType minimumRight) const
{
	if (User::checkUser(user) && user->getRoleRight(User::sysOpRight) < minimumRight) {
		throw ParameterException(ErrorException::ERROR_NOT_AUTHORIZED, "insufficient access rights for configuration cube", "user", (int)user->getId());
	}
...
RightsType ServerLogCube::getElementAccessRight(CPDatabase db, PUser user, CPDimension dim, IdentifierType elemId, vector<User::RoleCubeRight>& rcRights) const
{
	return User::checkUser(user) ? user->getRoleRight(User::sysOpRight) : RIGHT_SPLASH;
}
...
PCellStream ServerLogCube::evaluatePlan(PPlanNode plan, EngineBase::Type engineType) const
{
	PCellStream result(new ServerLogProcessor(plan->getArea()));
	return result;
}
