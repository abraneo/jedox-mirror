...
Context::Context() : svsChangeStatusContext(SVS_NONE), updateToken(true), refreshUsers(false), optimistic(true), worker(false), filteredFromMarkers(0), rulesContext(0), gpuOff(false), stopJob(false), ignoreStopJob(false), inJournal(false)
{
}
...
void Context::calcNewMarkerRules()
{
	...
	for (RuleIds::iterator i = newMarkerRules.begin(); i != newMarkerRules.end(); ++i) {
		...
		if (!cube) {
			Logger::debug << "calcNewMarkerRules: cube already deleted" << endl;
			continue;
		}
		PRule rule;
		try {
