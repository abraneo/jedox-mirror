...
RuleMarker::RuleMarker(dbID_cubeID allDbCube, const Area* fromArea, const Area* toArea) :
	numberTargetDimensions(0), fromDbCube(allDbCube), toDbCube(allDbCube), fromBase(new Area(fromArea->dimCount())), sourceArea(new Area(fromArea->dimCount())), mapping(0), isCons(false), isMulti(false), id(idHolder.getNewId())
{
...
RuleMarker::RuleMarker(dbID_cubeID fromDbCube, dbID_cubeID toDbCube, const vector<Node *>& path, const Area* toArea) :
	numberTargetDimensions(0), fromDbCube(fromDbCube), toDbCube(toDbCube), fromBase(), sourceArea(), mapping(0), isCons(false), isMulti(false), id(idHolder.getNewId())
{
	...
	for (vector<string>::iterator i = variables.begin(); i != variables.end(); ++i, ++j) {
		...
		Area::ConstDimIter a = toArea->dimBegin();
	...
	for (Area::ConstDimIter i = toArea->dimBegin(); i != toArea->dimEnd(); ++i, f++, ++dd) {
		const CPSet &s = *i;
...
ostream& operator <<(ostream& out, const RuleMarker& marker)
{
	...
	out << "MARKER from '" << fromCube->getName() << "' to '" << toCube->getName() << "': ";

	for (Area::ConstDimIter i = marker.fromBase->dimBegin(); i != marker.fromBase->dimEnd(); ++i) {
		const CPSet &s = *i;
