...
SystemDatabase::SystemDatabase(const SystemDatabase& d) :
	...
	groupPropertiesDimension(d.groupPropertiesDimension), roleDimension(d.roleDimension), rightObjectDimension(d.rightObjectDimension),
	...
	roleRolePropertiesCube(d.roleRolePropertiesCube), groupGroupPropertiesCube(d.groupGroupPropertiesCube),
	...
	groupPropertiesDimensionId(d.groupPropertiesDimensionId), roleDimensionId(d.roleDimensionId), rightObjectDimensionId(d.rightObjectDimensionId),
	...
	roleRolePropertiesCubeId(d.roleRolePropertiesCubeId), groupGroupPropertiesCubeId(d.groupGroupPropertiesCubeId),
...
void SystemDatabase::createSystemItems(PServer server, bool forceCreate)
{
	checkCheckedOut();
	extensible = true;

	// user dimension
	userDimension = checkAndCreateDimension(server, NAME_USER_DIMENSION);
	...
	userPropertiesDimension = checkAndCreateDimension(server, NAME_USER_PROPERTIES_DIMENSION);
	if (!userPropertiesDimension->isChangable()) {
	...
	// group dimension
	groupDimension = checkAndCreateDimension(server, NAME_GROUP_DIMENSION);
	...
	// role dimension
	roleDimension = checkAndCreateDimension(server, NAME_ROLE_DIMENSION);
	...
	rightObjectDimension = checkAndCreateDimension(server, NAME_RIGHT_OBJECT_DIMENSION);
	...
	PDimension sessionPropertiesDimension = checkAndCreateDimension(server, NAME_SESSION_PROPERTIES_DIMENSION, SESSION_PROPERTIES_ITEMS, SESSION_PROPERTIES_ITEMS+array_size(SESSION_PROPERTIES_ITEMS), true);
	PDimension jobPropertiesDimension = checkAndCreateDimension(server, NAME_JOB_PROPERTIES_DIMENSION, JOB_PROPERTIES_ITEMS, JOB_PROPERTIES_ITEMS+array_size(JOB_PROPERTIES_ITEMS), true);
	PDimension licensePropertiesDimension = checkAndCreateDimension(server, NAME_LICENSE_PROPERTIES_DIMENSION, LICENSE_PROPERTIES_ITEMS, LICENSE_PROPERTIES_ITEMS+array_size(LICENSE_PROPERTIES_ITEMS), true);

	/*PDimension cubePropertiesDimension = */checkAndCreateDimension(server, NAME_CUBE_PROPERTIES_DIMENSION);
	PDimension messageDimension = checkAndCreateDimension(server, NAME_MESSAGE_DIMENSION, MESSAGE_ITEMS, MESSAGE_ITEMS+array_size(MESSAGE_ITEMS));
	...
	//more dimensions
	groupPropertiesDimension = checkAndCreateDimension(server, NAME_GROUP_PROPERTIES_DIMENSION);
	checkAndCreateElement(server, groupPropertiesDimension, INACTIVE, Element::STRING, true);
	rolePropertiesDimension = checkAndCreateDimension(server, NAME_ROLE_PROPERTIES_DIMENSION);
...
PDimension SystemDatabase::checkAndCreateDimension(PServer server, const string& name, const ElementItem* elemBegin,  const ElementItem* elemEnd, bool rebuild)
{
...
PUser SystemDatabase::createExternalUser(PServer server, const string& name, vector<string>* groups)
{
	checkCheckedOut();
	...
	Element *userElement = checkAndReturnElement(server, userDimension, name, Element::STRING, true);
