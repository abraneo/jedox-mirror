...
public:
	...
	virtual void notifyAddDatabase(PServer server)
	{
	}
	...
	PCube findCube(IdentifierType identifier, PUser user, bool requireLoad, bool write) const
	{
		...
		if (cube->getType() == SYSTEMTYPE) {
			cube->checkCubeAccessRight(user, RIGHT_READ);
		}
...
private:
	...
	void checkDatabaseAccessRight(PUser user, RightsType minimumRight) const
	{
		if (User::checkUser(user))
			user->checkRoleRight(User::databaseRight, minimumRight);
	}

	void checkDimensionAccessRight(PUser user, RightsType minimumRight) const
	{
		if (User::checkUser(user))
			user->checkRoleRight(User::dimensionRight, minimumRight);
	}

	void checkCubeAccessRight(PUser user, RightsType minimumRight) const
	{
		if (User::checkUser(user))
			user->checkRoleRight(User::cubeRight, minimumRight);
	}

	void checkUserInfoAccessRight(PUser user, RightsType minimumRight) const
	{
		if (User::checkUser(user))
			user->checkRoleRight(User::userInfoRight, minimumRight);
	}

	void checkSystemOperationRight(PUser user, RightsType minimumRight) const
	{
		if (User::checkUser(user))
			user->checkRoleRight(User::sysOpRight, minimumRight);
	}
