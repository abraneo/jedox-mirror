...
#define GPU_RULES_DIFF_DIM 1
#define GPU_RULES_CONST 1
...
public:
	...
	Rule(PRuleNode ruleNode, PDatabase db, PCube cube, const string& definition, const string& external, const string& comment, time_t timestamp, bool activeRule) :
		Commitable(""), rule(ruleNode), comment(comment), external(external), timestamp(timestamp), activeRule(activeRule), isOptimized(false), restrictedRule(PExprNode()), restrictedDimension(0), evalCounter(0), evalNullCounter(0)
	...
	bool checkSimpleRuleNode(CPDatabase db, Node* n, Variability &varDimensions) const;

	bool isSimpleRule(CPDatabase db, Variability &varDimensions) const;
