...
bool NormalDimension::hasStringAttributes()
{
	...
	CPAttributesDimension ad = AttributedDimension::getAttributesDimension(db, getName());
	if (ad) {
		const ElementsType elements = ad->getElements(PUser(), false);
		for (ElementsType::const_iterator e = elements.begin(); e != elements.end(); ++e) {
