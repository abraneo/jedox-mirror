...
void PathTranslator::pathToBinPath(const IdentifiersType& path, GpuBinPath& binPath) const
{
	...
	for (uint32_t binIdx = 0; binIdx < binDimensionsCount.size(); binIdx++) {
		gpuBinType tmpIndex = 0;

		for (uint32_t i = 0; i < binDimensionsCount[binIdx]; i++, dimIdx++) {
			tmpIndex |= dimIdToBinDim(path[dimensionsMap[dimIdx]], dimIdx);
		}

		binPath.push_back(tmpIndex);
	}
}
...
void PathTranslator::binPathToPath(const GpuBinPath& binPath, IdentifiersType& path) const
{
	...
	path.clear();
	path.resize(dimensionsMap.size(), 0);
...
Set::Iterator::Iterator(const Set::Iterator &it) : it(it.it), pos(it.pos), par(it.par)
{
}

Set::Iterator::Iterator() : par(0)
{
}

Set::Iterator::Iterator(const SetType::iterator &it, const Set &s) : it(it), pos(it->first), par(&s)
{
}

Set::Iterator::Iterator(const SetType::iterator &it, IdentifierType pos, const Set &s) : it(it), pos(pos), par(&s)
{
}
...
{
	return Iterator(ranges.begin(), *this);
}

Set::Iterator Set::end() const
{
	return Iterator(ranges.end(), *this);
}
...
Area::PathIterator::PathIterator(const Area &area, bool end, const vector<ConstElemIter> &path) : path(path), end(end), area(&area)
{
	if (!end) {
...
Area::PathIterator::PathIterator() : end(true)
{
}

Area::PathIterator::PathIterator(const Area::PathIterator &it) : path(it.path), ids(it.ids), end(it.end), area(it.area)
{
}
...
void Area::PathIterator::set(size_t index, IdentifierType id)
{
	if (index >= 0 && index < ids.size()) {
		ids[index] = id;
		path[index] = area->find(index, id);
	} else {
		throw ErrorException(ErrorException::ERROR_INTERNAL, "index out of range");
	}
}
...
Area::Area(const IdentifiersType &path) : area(path.size()), areaSize(0), stepSizes(path.size(), 0)
{
...
Area::Area(const vector<IdentifiersType> &idsArea) : area(idsArea.size()), areaSize(0), stepSizes(idsArea.size(), 0)
{
	...
	for (size_t i = 0; i < size; i++) {
		...
		insert((IdentifierType)i, s);
	}
}

Area::ConstDimIter Area::dimBegin() const
{
	return area.begin();
}

Area::ConstDimIter Area::dimEnd() const
{
	return area.end();
}

Area::ConstElemIter Area::elemBegin(const ConstDimIter &it) const
{
	return (*it)->begin();
}

Area::ConstElemIter Area::elemEnd(const ConstDimIter &it) const
{
	return (*it)->end();
}
...
Area::PathIterator Area::pathBegin() const
{
	...
	for (ConstDimIter it = area.begin(); it != area.end(); ++it) {
		if (!(*it)->size()) {
			emptyArea = true;
			break;
		}
		path.push_back((*it)->begin());
	}
	return PathIterator(*this, emptyArea, path);
}
...
Area::ConstElemIter Area::find(ConstDimIter &it, IdentifierType elemId) const
{
	return (*it)->find(elemId);
}
...
void Area::clear(size_t dimOrdinal)
{
	area[dimOrdinal].reset(new Set);
	areaSize = 0;
}
...
size_t Area::elemCount(ConstDimIter &it) const
{
	return (*it)->size();
}
...
{
	return area[dimOrdinal] ? area[dimOrdinal]->size() : 0;
}
...
void CubeArea::splitbyTypes(SubCubeList &stringAreas, SubCubeList &numericAreas, SubCubeList &consolidatedAreas)
{
	if (!cube) {
		return;
	}
...
CPSet Area::getDim(size_t dimId) const
{
	return area[dimId];
}
...
{
	Area::ConstDimIter a1 = dimBegin();
	Area::ConstDimIter a2 = area.dimBegin();

	for (; a1 != dimEnd(); ++a1, ++a2) {
		if ((*a1) && (*a2) && (*a1)->size() > 0 && (*a2)->size() > 0) {
			if (!(*a1)->intersection((*a2).get(), 0, 0)) {
				return false;
			}
		}
	}
...
bool Area::operator==(const Area &other) const
{
	...
	Area::ConstDimIter a1 = dimBegin();
	Area::ConstDimIter a2 = other.dimBegin();
...
bool CubeArea::intersection(const Area &area, PCubeArea *intersection, SubCubeList *complementList) const
{
	...
	size_t dim = 0;
	for (; a1 != dimEnd(); ++a1, ++a2, ++interIt, ++complIt, ++dim) {
		if (!(*a1)->intersection((*a2).get(), intersection ? &*interIt : 0, complementList ? &*complIt : 0)) {
			return false;
		}
		if (intersection) {
...
bool CubeArea::reduction(vector<Set> &idsToRemove, PCubeArea *result, vector<IdentifiersType> &removedIds)
{
	size_t dims = dimCount();
	*result = PCubeArea(new CubeArea(db, cube, dims));
	removedIds.resize(dims);

	Area::ConstDimIter origIt = dimBegin();

	for (size_t i = 0; i < dims; ++i, ++origIt) {
		PSet newSet;
		if (idsToRemove[i].empty()) {
			newSet = PSet(new Set(*getDim(i)));
		} else {
			newSet = PSet(new Set());
			for (Set::Iterator it = (*origIt)->begin(); it != (*origIt)->end(); ++it) {
				if (idsToRemove[i].find(*it) == idsToRemove[i].end()) {
					newSet->insert(*it);
				} else {
					removedIds[i].push_back(*it);
				}
			}
			if (newSet->size() == 0) {
				return false;
...
bool CubeArea::operator==(const CubeArea &other) const
{
	return db == other.db && cube == other.cube && Area::operator==(other);
}
...
CubeArea::CubeArea(CPDatabase db, CPCube cube, const IdentifiersType &path) : Area(path), db(db), cube(cube)
{
	if (cube) {
		pathTranslator = cube->getPathTranslator();
	}
...
PCubeArea CubeArea::expandBase(AggregationMaps *aggregationMaps) const
{
	...
	size_t dimOrdinal = 0;
	for (ConstDimIter dit = dimBegin(); dit != dimEnd(); ++dit, ++didit) {
		...
		for (ConstElemIter eit = elemBegin(dit); eit != elemEnd(dit); ++eit) {
			Element *element = dimension->lookupElement(*eit, false);
			if (element->getBaseElementsCount() == 0) {
				s->insert(element->getIdentifier());
				if (aggregationMaps) {
					aggregationMaps->at(dimOrdinal).setBasesByParent(element->getIdentifier(), NULL);
				}
			} else {
				if (aggregationMaps) {
					aggregationMaps->at(dimOrdinal).setBasesByParent(element->getIdentifier(), element->getBaseElements());
				}
				if (singleElement) {
...
#endif
		if (aggregationMaps) {
			aggregationMaps->at(dimOrdinal).buildBaseToParentMap();
			aggregationMaps->at(dimOrdinal).setMergeMap(dimension->getMergeMap());
		}
		result->insert(dimOrdinal++, s);
	}
	return result;
}
			...
			case CHILDREN:
				{
					...
					if (!childE || childE->empty()) {
						...
						aggregationMaps[dimOrdinal].buildBaseToParentMap(element->getIdentifier(), childE);
			...
			case LEAVES:
				{
					...
					if (!baseE || baseE->empty()) {
						...
						aggregationMaps[dimOrdinal].buildBaseToParentMap(element->getIdentifier(), baseE);
...
PArea CubeArea::expandStar(ExpandStarType type) const
{
	...
	for (size_t i = 0; i < dimCount; i++) {
		...
		if (elemCount(i)) {
			...
			PSet s(new Set);
			PElementList elems = dimension->getElementList();
			for (ElementList::const_iterator eit = elems->begin(); eit != elems->end(); ++eit) {
				Element& elem = *eit;
				bool insert = false;
...
{
	ostr << '{';
	for (Area::ConstDimIter dim = cubeArea.dimBegin(); dim != cubeArea.dimEnd(); ++dim) {
		if (dim != cubeArea.dimBegin()) {
			ostr << 'x';
		}
...
PSubCubeList LockedCells::getLockedAreas()
{
	if (!lockedAreas && lockedPaths) {
		...
		for (SubCubeList::iterator scIt = lockedPathMerged.begin(); scIt != lockedPathMerged.end(); ++scIt) {
			PCubeArea expandedArea = scIt->second->expandBase(0);
