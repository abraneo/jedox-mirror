...
public:
	Planner(CPCube cube, PCubeArea area);
	PPlanNode createPlan(CubeArea::CellType cellType, RulesType rulesType, bool skipEmpty, uint64_t blockSize);
	void createRuleNodes(vector<PPlanNode> &nodes, RulesAreas &rulesAreas, const CellValue *outerDefaultValue, bool tryProcessors, bool allowMarkers);
private:
