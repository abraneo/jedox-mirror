...
public:
	...
	public:
		...
		Iterator();
		Iterator(const SetType::iterator &it, const Set &s);
...
public:
	...
	public:
		const_iterator(const SetType::iterator &it, const WeightedSet &s) : Iterator(it, s), parent(&s) {}
	...
	const_iterator begin() const {return const_iterator(ranges.begin(), *this);}
	const_iterator end() const {return const_iterator(ranges.end(), *this);}
...
class Area {
public:
	typedef vector<CPSet>::const_iterator ConstDimIter;
	...
	public:
		...
		void set(size_t index, IdentifierType id);
	private:
	...
	Area(const IdentifiersType &path);
	Area(const vector<IdentifiersType> &area);

	ConstDimIter dimBegin() const;
	ConstDimIter dimEnd() const;
	ConstElemIter elemBegin(const ConstDimIter &it) const;
	ConstElemIter elemEnd(const ConstDimIter &it) const;
...
public:
	...
	CubeArea(CPDatabase db, CPCube cube, const IdentifiersType &path);
	...
	void splitbyTypes(SubCubeList &stringAreas, SubCubeList &numericAreas, SubCubeList &consolidatedAreas);

	PCubeArea expandBase(AggregationMaps *aggregationMaps = 0) const;
	...
	bool reduction(vector<Set> &idsToRemove, PCubeArea *result, vector<IdentifiersType> &removedIds);
