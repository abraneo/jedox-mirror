	...
	if (!m_query_cache_full) {
		...
		if (!(m_querycache_writes++ % 1000)) {
			...
			if (total_size > m_qc_limit) {
				Logger::debug << "Per query cache full! Success ratio: " << (m_qc_lookups ? 100 * m_qc_hitcount / m_qc_lookups : 0) << " % null values: " << s_null_values << " double writes: " << m_querycache_writes * 100 / total_size - 100 << " %" << endl;				m_query_cache_full = true;
...
ERule::ERule(ECube *cube, CPRule rule) : cube(cube), nr_rule(rule->getId()), dest_area(0), is_base(0), is_cons(0), marker_flag(0), ubm_flag(0),
	ubm_rulesTested(0), ubm_noMoreRules(0),	bytecode(0), dbl_consts(0), str_consts(0), dbl_stack(0), str_stack(0), source_precalc(0), copy_mask(0),
	copy_source(0),	gc_bc_nr(0), gc_bc_max(0), gc_dbl_const_nr(0), gc_dbl_const_max(0), gc_str_const_nr(0), gc_str_const_max(0), gc_dbl_stack_nr(0),
	gc_dbl_stack_max(0), gc_str_stack_nr(0), gc_str_stack_max(0), gc_copy_nr(0), precalcStet(0), arule(rule.get())
{
}
...
void ERule::init()
{
	...
	dbl_stack = new double[gc_dbl_stack_max + 2];
	memset(dbl_stack, 0, sizeof(double) *(gc_dbl_stack_max + 2));
	str_stack = new string[gc_str_stack_max + 2];
}
...
ECube *NewEntryCube(const Cube &acube, const Database &db, const EngineBase &engine, Context *context)
{
	...
	result = new ECube((uint32_t)acube.getDimensions()->size(), (uint32_t)acube.getRules(PUser()).size(), acube, db);
	...
	try {
		...
		for (i = 0; i < result->nrRules; i++) {
			CPRule arule = acube.getRules(PUser()).at(i);
			if (arule->isActive()) {
