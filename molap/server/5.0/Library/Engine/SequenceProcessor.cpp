...
SequenceProcessor::SequenceProcessor(PEngineBase engine, CPPlanNode node) : engine(engine), children(node->getChildren()), currentStream(0), moveToNext(true), index(-1), pathTranslator(node->getArea()->getPathTranslator())
{
}
	...
	do {
		if (moveToNext) {
			...
			if (index + 1 < (int)children.size()) {
				++index;
				pStream = engine->createProcessor(children[index]);
