...
CubeFileStream::CubeFileStream(PDatabase db, PCube cube, FileReader *file, bool isString, map<IdentifierType, IdentifierType> *groupMap) :
	file(file), numeric(!isString), cube(cube), first(true), groupMap(groupMap), anyReMap(false)
{
	...
	do {
		if (file->isDataLine()) {
			...
			if (!failed) {
				...
				if (groupMap) {
					if (numeric) {
						failed = true; //there are no numeric entries in a rights cube
					} else {
						if (groupMap->find(path[0]) == groupMap->end()) {
							failed = true;
						} else {
							iFirst = 1;
						}
			...
			if (!failed) {
				for (size_t i = iFirst; i < dims.size(); i++) {
					if (first || prev[i] != path[i]) {
