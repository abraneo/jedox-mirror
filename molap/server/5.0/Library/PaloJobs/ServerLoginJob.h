...
private:
	...
	PSystemDatabase checkCreateUser(bool external, string& username, vector<string>* groups) {
		...
		if (!sd->userExist(username, external) || (external && (sd->groupsUpdateRequired(server, username, groups)))) {
			...
			for (int i = 0; i < Commitable::COMMIT_REPEATS; i++) {
				...
				dbs->set(sd);
				if (external) {
					sd->createExternalUser(COMMITABLE_CAST(Server, server), username, groups);
				} else {
	...
	boost::shared_ptr<PaloSession> loginInternal(bool useMd5, string &sudoName) {
		...
		return PaloSession::createSession(user, false, server->getDefaultTtl(), server->useShortSid(), jobRequest->type == 1, address, &machine, &required, &optional, description);
	}
	...
	boost::shared_ptr<PaloSession> loginExternalInformation(string &sudoName) {
		...
		return PaloSession::createSession(user, false, server->getDefaultTtl(), server->useShortSid(), jobRequest->type == 1, getIoTask() ? getIoTask()->getPeerName() : string(), &machine, &required, &optional, description);
	}
	...
	boost::shared_ptr<PaloSession> loginWindowsAuthentication(string domain, string username, vector<string> &winGroups) {
		...
		sd = checkCreateUser(true, fullName, &groups); // possible checkout of sd
		if (sd == 0) {
			throw ErrorException(ErrorException::ERROR_DATABASE_NOT_FOUND, "system database not found");
		}
	...
	boost::shared_ptr<PaloSession> loginExternalAuthorization(string &sudoName) {
		...
		PSystemDatabase sd = checkCreateUser(true, *username, &groups);
		if (!sudoName.empty()) {
			sd = checkCreateUser(false, sudoName, 0);
		}
		if (sd == 0) {
