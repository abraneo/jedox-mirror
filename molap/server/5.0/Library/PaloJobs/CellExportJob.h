	...
	void compute() {
		...
		do {
			...
			for (it = larea.begin(); it != larea.end(); ++it) {
				PCubeArea calcArea = checkRights(rcRights, checkPermissions, *it, 0);
...
//					cout << *calcArea << endl;
				...
				if (cs) { // empty plan for whatever reason
					moreValues = loop(*it, calcArea, cs, &freeCount, true, props, mySkipEmpty, rcRights, &lastKey);
					if (!freeCount) {
						++it;
						break;
					}
			...
			startPath = CPArea(new Area(lastKey));
