	...
	void compute() {
		...
		try {
			for (int commitTry = 0; commitTry < Commitable::COMMIT_REPEATS; ++commitTry) {
				...
				// TODO: gpu license activation should be tested
				if(server->getNumberofGPUs() > 0) {
					PEngineBase gpuEngine = server->getEngine(EngineBase::GPU, false);
					if(gpuEngine != NULL ) {
						server->removeEngine(gpuEngine);
					}
					vector<string> noDeviceIds;
					server->activateGpuEngine(noDeviceIds);
				}
