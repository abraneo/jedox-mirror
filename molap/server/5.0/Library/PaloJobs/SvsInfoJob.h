...
public:
	...
	void compute() {
		...
		int svsActive = loginWorker ? 1 : 0;
		int loginMode = svsActive ? server->getLoginType() : 0;
		int cubeWorker = CubeWorker::useCubeWorker() ? 1 : 0;
		int drillThroughEnabled = svsActive ? server->isDrillThroughEnabled() : 0;
		int dimWorkerActive = dimensionWorker ? 1 : 0;
