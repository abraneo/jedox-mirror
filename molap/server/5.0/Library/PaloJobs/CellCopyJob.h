...
private:
	...
	IdentifierType checkArea()
	{
		...
		Area::ConstDimIter dit = cellPathTo->dimBegin();
		for (size_t i = 0; i < size; i++, ++dit) {
	...
	double calculatePredictFunction(PCubeArea calcArea, IdentifierType timeDim)
	{
		...
		if (calcArea->getSize()) {
			...
			PCellStream cs = cube->calculateArea(calcArea, CubeArea::NUMERIC, jobRequest->useRules ? ALL_RULES : NO_RULES, false, UNLIMITED_SORTED_PLAN);
			while (cs->next()) {
