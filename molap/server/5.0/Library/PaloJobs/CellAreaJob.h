...
public:
	...
	void compute() {
		...
		double cellCount = fillEmptyDim(rcRights, checkPermissions);
		...
		if (area->getSize()) {
			PCubeArea calcArea = checkRights(rcRights, checkPermissions, area, isSum ? 0 : &strElem);
			if (!isSum && strElem) {
			...
			if (calcArea->getSize()) {
				if (isSum) {
					...
					if (plan->getType() == SEQUENCE) {
						children = plan->getChildren();
					} else {
						children.push_back(plan);
					}
					PPlanNode aggregationNode = PPlanNode(new AggregationPlanNode(calcArea, 0, children, aggregationMaps, 0, CPCube(), aggrType));
					cs = cube->evaluatePlan(aggregationNode, EngineBase::ANY);
				}
			...
			loop(area, calcArea, cs, &maxCC, true, props, false, rcRights, 0); //calls appendValue
...
private:
	...
	void generateResponse() {
		for (StreamToAreaMapper::Results::iterator it = mapper.results.begin(); it != mapper.results.end(); ++it) {
