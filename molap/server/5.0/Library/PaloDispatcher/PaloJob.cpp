...
void PaloJob::generateDatabasesResponse(CPServer server, vector<CPDatabase>* databases, bool showNormal, bool showSystem, bool showUserInfo)
{
	response = new HttpResponse(HttpResponse::OK);
	StringBuffer& body = response->getBody();
	...
	for (vector<CPDatabase>::iterator i = databases->begin(); i != databases->end(); ++i) {
		...
		bool normal = (database->getType() == NORMALTYPE) && showNormal;
		bool system = (database->getType() == SYSTEMTYPE) && showSystem;
		bool userinfo = (database->getType() == USER_INFOTYPE) && showUserInfo;
...
void PaloJob::generateElementResponse(CPDimension dimension, Element* element)
{
	response = new HttpResponse(HttpResponse::OK);
	StringBuffer& body = response->getBody();
...
void PaloJob::generateElementsResponse(CPDimension dimension, ElementsType* elements)
{
...
void PaloJob::generateElementsValuesResponse(CPDimension dimension, list<Element*> *elements, IdentifiersType &elementIds, vector<CellValue> *vals)
{
	response = new HttpResponse(HttpResponse::OK);
	StringBuffer& body = response->getBody();
	...
	vector<CellValue>::iterator vit = vals->begin();
	if (elements && !elements->empty()) {
		for (list<Element*>::iterator i = elements->begin(); i != elements->end(); ++i, ++vit) {
			Element* element = (*i);

			appendElement(&body, dimension, element);
			appendCell(body, *vit, false, false, false, prop_vals);
		}
	} else {
		for (IdentifiersType::iterator elIdIt = elementIds.begin(); elIdIt != elementIds.end(); ++elIdIt, ++vit) {
			appendElement(&body, dimension, 0, *elIdIt);
			appendCell(body, *vit, false, false, false, prop_vals);
		}
	}
}

void PaloJob::generateElementLockResponse(CPDimension dimension, Element *element)
{
	response = new HttpResponse(HttpResponse::OK);
	StringBuffer& body = response->getBody();
	...
	const WeightedSet *elems = element->getBaseElements();

	if (elems && elems->size() > 0) {
		bool bFirst = true;
		for (WeightedSet::const_iterator it = elems->begin(); it != elems->end(); ++it) {
			if (!bFirst) {
				body.appendChar(',');
			}
			bFirst = false;
			body.appendInteger(it.first());
		}
	} else {
		body.appendInteger(element->getIdentifier());
	}
			...
			for (size_t i = 0; i < numDimensions; i++) {
				try {
					if (dims[i]->getDimensionType() == Dimension::VIRTUAL) {
						...
						path[i] = dims[i]->findElement(it.at(i), 0, false)->getIdentifier();
					}
	...
	if (jobRequest->paths) {
		...
		for (size_t j = 0; j < jobRequest->pathsName->size(); j++) {
			...
			if (names.size() != numDimensions) {
				...
				for (size_t i = 0; i < numDimensions; i++) {
					try {
						path[i] = dims[i]->findElementByName(names.at(i), 0, false)->getIdentifier();
					} catch (ErrorException &e) {
						if (invalidPaths && (e.getErrorType() == ErrorException::ERROR_ELEMENT_NOT_FOUND)) {
							invalidPaths->insert(j);
							path[i] = -1;
...
void PaloJob::appendDimension(StringBuffer* sb, CPDimension dimension)
{
...
void PaloJob::appendElement(StringBuffer* sb, CPDimension dimension, Element* element, IdentifierType elemId)
{
	if (element) {
		...
		sb->appendCsvInteger((int32_t)element->getIndent());
		sb->appendCsvInteger((int32_t)element->getDepth());
...
{
	//1:2:3,4:5:6,...
	for (Area::ConstDimIter dit = area->dimBegin(); dit != area->dimEnd(); ++dit) {
		if (dit != area->dimBegin())
			sb->appendChar(',');
