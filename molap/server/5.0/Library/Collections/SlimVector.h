...
template <typename TType> SlimVector<TType>::SlimVector(uint32_t pageMemorySize = 4096) :
...
template <typename TType> uint32_t SlimVector<TType>::push_back(vector<TType> &v, TType empty, bool &pageAdded, bool &pageCheckout)
{
	...
	if (!pages.empty()) {
		...
		if (lastPage->endSpace() < v.size()) {
			...
			emptyAdded = lastPage->fillRest(empty); // fill with "empty" TType to the rest of the page (not enough space here)
