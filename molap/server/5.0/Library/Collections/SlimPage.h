...
template <typename TType> SlimPage<TType>::SlimPage(uint32_t size) :
	maxBlockCount(size / sizeof(TType)), blocksFilled(0), checkedOut(true)
{
...
template <typename TType> SlimPage<TType>::SlimPage(const SlimPage & page) :
	maxBlockCount(page.maxBlockCount), blocksFilled(page.blocksFilled), checkedOut(true)
{
...
template <typename TType> uint32_t SlimPage<TType>::fillRest(TType value)
{
	checkCheckedOut();

	uint32_t filled = maxBlockCount - blocksFilled;
	memset(&blocks[blocksFilled], value, sizeof(TType) * filled);
	blocksFilled = maxBlockCount;
	return filled;
}
