...
public:
	...
	public:
		Reader(DoubleVectorMap &cvm) : started(false), pcvm(cvm.shared_from_this()), cvm(cvm), iter(cvm.begin()) {}
		virtual ~Reader() {}
		virtual bool next() {
			if (iter != cvm.end()) {
				started = true;
...
public:
	...
	class Reader : public CellValueStream {
	public:
		Reader(DoubleArrayMap &cam) : started(false), pcam(cam.shared_from_this()), cam(cam), iter(cam.begin()), vkey(_N) {}
		virtual ~Reader() {}
		virtual bool next() {
			if (iter != cam.end()) {
