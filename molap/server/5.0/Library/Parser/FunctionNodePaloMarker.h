...
public:
	...
	RuleValueType getValue(const CubeArea *cellPath, bool* isCachable, RulesContext* mem_context) {
		...
		if (valid) {
			...
			PCellStream cs = cube->calculateArea(calcArea, CubeArea::ALL, ALL_RULES, false, UNLIMITED_UNSORTED_PLAN);
			if (cs->next()) {
				value = cs->getValue();
			}
			if (!value.isEmpty()) {
	...
	bool genCode(bytecode_generator& generator, uint8_t want) const {
		...
		if (params->size() > 2 && params->at(0)->isConstant(nullCube) && params->at(1)->isConstant(nullCube)) {
