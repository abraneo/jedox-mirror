...
public:
	...
	map<Element*, string> computeDimensionTransformations(CPDatabase db, CPCube cube) {
		...
		if (left->isDimensionTransformation(cube, &dimensionId) && right->isConstant(cube)) {
			...
			return elements;
		} else if (right->isDimensionTransformation(cube, &dimensionId) && left->isConstant(cube)) {
