...
public:
	FunctionNodePaloEchildcount() :
		FunctionNodePalo() {
	}

	FunctionNodePaloEchildcount(const string& name, vector<Node*> *params) :
...
public:
	...
	RuleValueType getValue(const CubeArea* cellPath, bool* isCachable, RulesContext* mem_context) {
		...
		if (valid) {
			...
			if (element) {
				result.doubleValue = (double)element->getChildrenCount();
				return result;
			}
		}
	...
	bool genCode(bytecode_generator& generator, uint8_t want) const {
		...
		if (!params->at(0)->genCode(generator, Node::NODE_STRING))
			return false;
		if (!generator.EmitFuncCode("palo.echildcount"))
			return false;
