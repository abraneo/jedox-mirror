...
public:
	FunctionNodePaloEtype() :
		FunctionNodePalo() {
	}

	FunctionNodePaloEtype(const string& name, vector<Node*> *params) :
...
public:
	...
	RuleValueType getValue(const CubeArea* cellPath, bool* isCachable, RulesContext* mem_context) {
		...
		if (valid) {
			...
			if (element) {
				Element::Type type = element->getElementType();
	...
	bool genCode(bytecode_generator& generator, uint8_t want) const {
		...
		if (!params->at(0)->genCode(generator, Node::NODE_STRING))
			return false;
		if (!generator.EmitFuncCode("palo.etype"))
			return false;
