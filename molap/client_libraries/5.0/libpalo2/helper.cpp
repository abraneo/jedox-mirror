...
void add2ConditionString2(const char *BoolOp, const char *CmpOp, struct arg_palo_value_m *Val, std::string& conditionstring) {
...
void add2ConditionString(char *BoolOp, compare_op cmp, struct arg_palo_value_m *Val, std::string& conditionstring) {
	switch (cmp) {
		case compare_op_lt:
			// <
			add2ConditionString2(BoolOp, "%3C", Val, conditionstring);
			break;
		case compare_op_gt:
			// >
			add2ConditionString2(BoolOp, "%3E", Val, conditionstring);
			break;
		case compare_op_lte:
			// <=
			add2ConditionString2(BoolOp, "%3C%3D", Val, conditionstring);
			break;
		case compare_op_gte:
			// >=
			add2ConditionString2(BoolOp, "%3E%3D", Val, conditionstring);
			break;
		case compare_op_eq:
			// ==
			add2ConditionString2(BoolOp, "%3D%3D", Val, conditionstring);
			break;
		case compare_op_neq:
			// !=
			add2ConditionString2(BoolOp, "%21%3D", Val, conditionstring);
			break;
	}
}
...
void getConditionString(struct arg_getdata_export_options_filter_m *Filter, std::string& conditionstring) {
	...
	if (!conditionstring.empty() && (Filter->cmp2 != compare_op_true) && (Filter->cmp2 != compare_op_unknown)) {
		switch (Filter->andor12) {
			case bool_op_and:
				BoolOp = const_cast<char *>("%20and%20");
				break;
			case bool_op_or:
				BoolOp = const_cast<char *>("%20or%20");
				break;
			case bool_op_xor:
				BoolOp = const_cast<char *>("%20xor%20");
				break;
			default:
				break;
		}
