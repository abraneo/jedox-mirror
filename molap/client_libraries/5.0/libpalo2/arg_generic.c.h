...
void PALO_ARG_FUNC(free_arg_parent_info_contents)(struct arg_parent_info *pi) {
	if (pi != NULL) {
		PALO_ARG_FUNC(free_arg_str)(pi->name);
	}
}


void PALO_ARG_FUNC(free_arg_child_info_contents)(struct arg_child_info *ci) {
	if (ci != NULL) {
...
void PALO_ARG_FUNC(free_arg_dim_element_info2_contents)(struct arg_dim_element_info2 *dei) {
	size_t i, count;

	if (dei != NULL) {
		PALO_ARG_FUNC(free_arg_str)(dei->name);
		if (dei->parents != NULL) {
			count = dei->num_parents;
			for (i = 0; i < count; i++) {
				PALO_ARG_FUNC(free_arg_parent_info_contents)(dei->parents + i);
			}
			free(dei->parents);
			dei->parents = NULL;
		}
		if (dei->children != NULL) {
			count = dei->num_children;
			for (i = 0; i < count; i++) {
				PALO_ARG_FUNC(free_arg_child_info_contents)(dei->children + i);
			}
			free(dei->children);
			dei->children = NULL;
		}
...
void PALO_ARG_FUNC(free_arg_dim_element_info2_array_contents)(struct arg_dim_element_info2_array *deia) {
	size_t i;

	if (deia != NULL) {
		if (deia->a != NULL) {
			for (i = 0; i < deia->len; i++) {
				PALO_ARG_FUNC(free_arg_dim_element_info2_contents)(deia->a + i);
			}
			free(deia->a);
			deia->a = NULL;
		}
