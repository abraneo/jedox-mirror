...
palo_err PALO_SUFFIX(set_element_info2)(wchar_t **errmsg, struct arg_dim_element_info2 *retresult, struct conversions *convs, const jedox::palo::ELEMENT_INFO &ei, Dimension &dim) {
	palo_err result = PALO_SUCCESS;
	size_t j, k, vsize;
	long identifier;
	...
	k = 0;
	if (vsize > 0) {
		retresult->parents = (struct arg_parent_info*)calloc(vsize, sizeof(struct arg_parent_info));
		if (retresult->parents == NULL) {
			*errmsg = MSG_PALO_ERR_NO_MEM;
			return PALO_ERR_NO_MEM;
		}
		...
		for (j = 0;j < vsize;j++) {
			identifier = ei.parents[j];

			if (dim.Exists(identifier, true)) {

				(retresult->parents + k)->identifier = identifier;

				result = UTF8_TO_STR(convs, &((retresult->parents + k)->name), dim[identifier].getCacheData().nelement.c_str());
				if (result != PALO_SUCCESS) {
					*errmsg = MSG_ENCODING_FAILED;
					return result;
				}

				result = number2types(dim[identifier].getCacheData().type, &((retresult->parents + k)->type));
				if (result != PALO_SUCCESS) {
					*errmsg = MSG_TYPE_CONVERSION_FAILED;
					return result;
				}
		...
		if (k == 0) {
			free(retresult->parents);
			retresult->parents = NULL;
		} else {
			if (k < vsize) {
				retresult->parents = (arg_parent_info*)realloc(retresult->parents, k * sizeof(struct arg_parent_info));
			}
		}
	} else {
		retresult->parents = NULL;
	...
	retresult->num_parents = k;

	vsize = ei.children.size();

	k = 0;
	if (vsize > 0) {
		retresult->children = (struct arg_child_info*)calloc(vsize, sizeof(struct arg_child_info));
		if (retresult->children == NULL) {
			*errmsg = MSG_PALO_ERR_NO_MEM;
			return result;
		}
		...
		for (j = 0;j < vsize;j++) {
			identifier = ei.children[j];

			if (dim.Exists(identifier, true)) {

				(retresult->children + k)->identifier = identifier;
				(retresult->children + k)->factor = ei.weights[j];

				result = UTF8_TO_STR(convs, &((retresult->children + k)->name), dim[identifier].getCacheData().nelement.c_str());
				if (result != PALO_SUCCESS) {
					*errmsg = MSG_ENCODING_FAILED;
					return result;
				}

				result = number2types(dim[identifier].getCacheData().type, &((retresult->children + k)->type));
				if (result != PALO_SUCCESS) {
					*errmsg = MSG_TYPE_CONVERSION_FAILED;
					return result;
				}
		...
		if (k == 0) {
			free(retresult->children);
			retresult->children = NULL;
		} else {
			if (k < vsize) {
				retresult->children = (arg_child_info*)realloc(retresult->children, k * sizeof(struct arg_child_info));
			}
		}
	} else {
		retresult->children = NULL;
...
palo_err PALO_SUFFIX(element_info)(struct errstack *errs, struct arg_dim_element_info2 *retresult, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *dimension, const CHAR_T *element) {
	palo_err result = PALO_SUCCESS;
	wchar_t *errmsg = NULL;
	...
	try {
		...
		result = PALO_SUFFIX(set_element_info2)(&errmsg, retresult, convs, dim[utf8_element].getCacheDataCopy(), dim);
	} catch (const SocketException& exp) {
		if (utf82wcs(convs, &errmsg, exp.what()) != PALO_SUCCESS) {
			errmsg = NULL;
		}
	...
	if (result != PALO_SUCCESS) {
		PALO_SUFFIX(free_arg_dim_element_info2_contents)(retresult);
		ERRSTACK_RETURN(errs, result, __FUNCTION__, __LINE__, __FILE__, errmsg);
	}
...
palo_err PALO_SUFFIX(dimension_list_elements2)(struct errstack *errs, struct arg_dim_element_info2_array *retresult, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *dimension) {
	palo_err result = PALO_SUCCESS;
	wchar_t *errmsg = NULL;
	char *utf8_db = NULL, *utf8_dim = NULL;
	size_t elemcount = 0, pos = 0;
	...
	try {
		...
		if (elemcount > 0) {
			retresult->a = (struct arg_dim_element_info2*)calloc(elemcount, sizeof(struct arg_dim_element_info2));
			...
			if (retresult->a != NULL) {
				...
				while (!(*it).end()) {
					...
					result = PALO_SUFFIX(set_element_info2)(&errmsg, retresult->a + pos, convs, **it, dim);
					if (result != PALO_SUCCESS) {
						break;
					}
	...
	if (result != PALO_SUCCESS) {
		PALO_SUFFIX(free_arg_dim_element_info2_array_contents)(retresult);
		ERRSTACK_RETURN(errs, result, __FUNCTION__, __LINE__, __FILE__, errmsg);
	}
...
palo_err PALO_SUFFIX(_getdata_multi)(struct errstack *errs, struct arg_palo_value_array *retresult, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, const struct arg_str_array_2d *coordinates) {
...
palo_err PALO_SUFFIX(_setdata_multi)(struct errstack *errs, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, const struct arg_palo_dataset_array *dsa, splash_mode mode, unsigned short add, unsigned short eventprocessor) {
	...
	if (result == PALO_SUCCESS) {
		try {
			(*(so->myServer))[utf8_db].cube[utf8_cube].CellReplaceBulk(coord, cvc, splashmode, (add == 0) ? false : true, (eventprocessor == 0) ? false : true);
		} catch (const SocketException& exp) {
			if (utf82wcs(convs, &errmsg, exp.what()) != PALO_SUCCESS) {
				errmsg = NULL;
			}
...
palo_err PALO_SUFFIX(setdata)(struct errstack *errs, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, const struct arg_palo_dataset *ds, splash_mode mode) {
	...
	wchar_t *errmsg = NULL;
	char *utf8_db = NULL, *utf8_cube = NULL, *name;
	size_t vsize = ds->coordinates.len, i;
	vector<string> elements(vsize);
	CELL_VALUE cv;
	SPLASH_MODE splashmode;
	...
	if (result == PALO_SUCCESS) {
		try {
			(*(so->myServer))[utf8_db].cube[utf8_cube].CellReplace(elements, cv, splashmode);
		} catch (const SocketException& exp) {
			if (utf82wcs(convs, &errmsg, exp.what()) != PALO_SUCCESS) {
				errmsg = NULL;
			}
...
palo_err PALO_SUFFIX(getdata_export_rule)(struct errstack *errs, struct arg_getdata_export_result *retresult, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, const struct arg_str_array_array *elements, struct arg_getdata_export_options *options, unsigned short use_rules) {
	palo_err result = PALO_SUCCESS;
	wchar_t *errmsg = NULL;
	char *name = NULL;
	size_t vsize = elements->len, vsize2, i, j , numdim;
...
palo_err PALO_SUFFIX(getdata_export)(struct errstack *errs, struct arg_getdata_export_result *retresult, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, const struct arg_str_array_array *elements, struct arg_getdata_export_options *options) {
	return PALO_SUFFIX(getdata_export_rule)(errs, retresult, so, convs, database, cube, elements, options, false);
}
...
palo_err PALO_SUFFIX(cell_copy)(struct errstack *errs, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, const struct arg_str_array *from, const struct arg_str_array *to, unsigned short withValue, double val) {
	...
	vsize = from->len;
	vector<string> fromelements(vsize);
	...
	if (result == PALO_SUCCESS) {
		try {
			if (withValue == 1) {
				(*(so->myServer))[utf8_db].cube[utf8_cube].CellCopy(fromelements, toelements, val);
			} else {
				(*(so->myServer))[utf8_db].cube[utf8_cube].CellCopy(fromelements, toelements);
			}
		} catch (const SocketException& exp) {
			if (utf82wcs(convs, &errmsg, exp.what()) != PALO_SUCCESS) {
				errmsg = NULL;
...
palo_err PALO_SUFFIX(rule_add)(struct errstack *errs, struct arg_rule_info *retresult, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, const CHAR_T *definition, unsigned short use_identifier, const CHAR_T *extern_id, const CHAR_T *comment, unsigned short activate) {
	palo_err result = PALO_SUCCESS;
	wchar_t *errmsg = NULL;
	char *name = NULL;
	...
	try {
		RULE_INFO ri = (*(so->myServer))[utf8_db].cube[utf8_cube].RuleCreate(utf8_definition, use_identifier, utf8_extern_id, utf8_comment, activate);
...
palo_err PALO_SUFFIX(rule_modify)(struct errstack *errs, struct arg_rule_info *retresult, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, long id, const CHAR_T *definition, unsigned short use_identifier, const CHAR_T *extern_id, const CHAR_T *comment, unsigned short activate) {
	palo_err result = PALO_SUCCESS;
	wchar_t *errmsg = NULL;
	char *name = NULL;
	...
	try {
		RULE_INFO ri = (*(so->myServer))[utf8_db].cube[utf8_cube].RuleModify(id, utf8_definition, use_identifier, utf8_extern_id, utf8_comment, activate);
		result = PALO_SUFFIX(set_rule_info)(&errmsg, retresult, convs, ri);
	} catch (const SocketException& exp) {
		if (utf82wcs(convs, &errmsg, exp.what()) != PALO_SUCCESS) {
			errmsg = NULL;
...
palo_err PALO_SUFFIX(rule_parse)(struct errstack *errs, CHAR_T **retresult, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, const CHAR_T *definition) {
	palo_err result = PALO_SUCCESS;
	wchar_t *errmsg = NULL;
	char *utf8_db = NULL, *utf8_cube = NULL, *utf8_definition = NULL;
	...
	result = STR_TO_UTF8(convs, &utf8_definition, definition);
	if (result != PALO_SUCCESS) {
		free(utf8_db);
		free(utf8_cube);
		ERRSTACK_RETURN(errs, result, __FUNCTION__, __LINE__, __FILE__, MSG_ENCODING_FAILED);
...
palo_err PALO_SUFFIX(cube_lock)(struct errstack *errs, struct arg_lock_info *retresult, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube,  const struct arg_str_array_array *coordinates) {
	palo_err result = PALO_SUCCESS;
	wchar_t *errmsg = NULL;
	char *utf8_db = NULL, *utf8_cube = NULL, *name;
...
palo_err PALO_SUFFIX(cell_goalseek)(struct errstack *errs, struct sock_obj *so, struct conversions *convs, const CHAR_T *database, const CHAR_T *cube, const struct arg_str_array *path, double val) {
	...
	if (result == PALO_SUCCESS) {
		try {
			(*(so->myServer))[utf8_db].cube[utf8_cube].CellGoalSeek(elements, val);
		} catch (const SocketException& exp) {
			if (utf82wcs(convs, &errmsg, exp.what()) != PALO_SUCCESS) {
				errmsg = NULL;
			}
