...
public:
	...
	struct QueryCacheInnerIndex {
		...
		StringArray path;
		StringArray pathOrigcase; // TODO (scharfes s problem)
	...
	struct HashIndex {
		size_t operator()(const QueryCacheIndex &i) const
		{
			return i.operator size_t();
		}
	};

	struct HashInnerIndex {
		size_t operator()(const QueryCacheInnerIndex &i) const
		{
			return i.operator size_t();
		}
	};
...
private:
	typedef std::unordered_map<QueryCacheInnerIndex, QueryCacheEntry, HashInnerIndex> QueryCacheInnerMap;
	typedef std::unordered_map<QueryCacheIndex, QueryCacheInnerMap, HashIndex> QueryCacheMap;
