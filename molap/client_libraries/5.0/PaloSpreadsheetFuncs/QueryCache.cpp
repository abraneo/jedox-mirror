...
QueryCache::QueryCacheIndex::operator size_t() const
{
	return boost::hash_value(server_srv.get()) + 81 * boost::hash_value(database) + 2401 * boost::hash_value(cube);
}

QueryCache::QueryCacheInnerIndex::operator size_t() const
{
	size_t hash = 0;

	for (StringArray::const_iterator it = path.begin(); it != path.end(); ++it) {
		hash += boost::hash_value(*it);
	}
...
QueryCache::QueryCacheIndex::QueryCacheIndex(boost::shared_ptr<jedox::palo::Server> s, const std::string& db, const std::string& c) :
		server_srv(s), database(jedox::util::UTF8Comparer::toUpper(db)), cube(jedox::util::UTF8Comparer::toUpper(c))
{
}
...
QueryCache::QueryCacheInnerIndex::QueryCacheInnerIndex(const StringArray& p)
{
	...
	for (StringArray::const_iterator i = p.begin(); i != p.end(); i++) {
		...
		path.push_back(jedox::util::UTF8Comparer::toUpper(*i));
	}
}
...
bool QueryCache::QueryCacheIndex::operator==(const QueryCacheIndex &r) const
{
	if (server_srv == r.server_srv) {
		if (!database.compare(r.database)) {
			return !cube.compare(r.cube);
		}
	}
	return false;
}
...
{
	StringArray::const_iterator lit = path.begin();
	StringArray::const_iterator rit = r.path.begin();

	for (; (lit != path.end()) && (rit != r.path.end()); ++lit, ++rit) {
		if (lit->compare(*rit)) {
			return false;
		}
	}
