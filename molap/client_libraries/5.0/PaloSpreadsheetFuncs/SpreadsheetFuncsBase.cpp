...
void SpreadsheetFuncsBase::CellCopyWrapper(boost::shared_ptr<jedox::palo::Server> s, const long dbid, const long cid, const StringArray& from, const StringArray& to, const double* value, const StringArrayArray& LockingArea)
{
	if (LockingArea.size() == 0) {
		if (value != NULL) {
			(*s)[dbid].cube[cid].CellCopy(from, to, *value);
		} else {
			(*s)[dbid].cube[cid].CellCopy(from, to);
		}
	} else {
		if (value != NULL) {
			(*s)[dbid].cube[cid].CellCopy(from, to, *value, LockingArea);
		} else {
			(*s)[dbid].cube[cid].CellCopy(from, to, LockingArea);
		}
	}
}
		...
		if (cur.type == CellValue::STRING) {
			...
			if ((tsize == 1) && (val.length() > 2) && ((val[0] == '#') && (val[1] == '#'))) {
				if (val[val.length() - 1] == '%') {
					...
					double scaled_val = cur.val.d * ((100.0 + _string2Double(value.val.s.substr(2, val.length() - 3))) / 100.0);
				...
				CellReplaceWrapper(s, dbid, cid, path, _double2CELL_VALUE(_string2Double(value.val.s.substr(1))), MODE_SPLASH_DEFAULT, LockingArea);
			} else if ((tsize == 1) && (val.length() > 1) && ((val[0] == '!') && (val[1] != '!'))) {
				// base set "!..."
				CellReplaceWrapper(s, dbid, cid, path, _double2CELL_VALUE(_string2Double(value.val.s.substr(1))), MODE_SPLASH_SET, LockingArea);
			} else if ((tsize == 1) && (val.length() > 2) && ((val[0] == '!') && (val[1] == '!'))) {
				// base add "!!..."
				CellReplaceWrapper(s, dbid, cid, path, _double2CELL_VALUE(_string2Double(value.val.s.substr(2))), MODE_SPLASH_ADD, LockingArea);
			} else if ((tsize == 1) && (val.length() > 2) && ((val[0] == '#') && (val[val.length() - 1] == '%'))) {
				// splashing with percentage
				double scaled_val = cur.val.d * ((_string2Double(value.val.s.substr(1, val.length() - 2))) / 100.0);
				CellReplaceWrapper(s, dbid, cid, path, _double2CELL_VALUE(scaled_val), MODE_SPLASH_DEFAULT, LockingArea);
			} else if ((tsize == 1) && (val.length() > 1) && ((val[0] == '?'))) {
				//Goalseek
				StringArrayArray dummy;
				FPaloGoalSeek(s, database, cube, path, _string2Double(value.val.s.substr(1)), GoalSeekComplete, dummy);
			} else {
				// copy / like
				if (((tsize == 3) && (jedox::util::StringUtils::toUpper(tokens[1]) == "LIKE")) || ((tsize == 2) && (jedox::util::StringUtils::toUpper(tokens[0]) == "COPY"))) {
					double d;
					string coord_str;
					bool is_like, is_percentage = false, is_add = false;
					if (tsize == 3) {
						if (tokens[0].c_str()[0] == '#') {
							if (tokens[0].c_str()[1] == '#') {
								d = _string2Double(tokens[0].substr(2), is_percentage);
								is_add = true;
							} else {
					...
					for (size_t i = 0; i < coords.size(); i++) {
						...
						if (t.size() == 1) {
							...
							for (i = cubeDims.begin(); i != cubeDims.end(); i++) {
								if (jedox::util::StringUtils::toUpper(*i) == jedox::util::StringUtils::toUpper(t[0])) {
									break;
								}
							}
							if (i == cubeDims.end()) {
					...
					CellCopyWrapper(s, dbid, cid, realCoords, path, &d, LockingArea);
				} else {
					CellReplaceWrapper(s, dbid, cid, path, _double2CELL_VALUE(_string2Double(value.val.s)), splash, LockingArea);
				}
			}
