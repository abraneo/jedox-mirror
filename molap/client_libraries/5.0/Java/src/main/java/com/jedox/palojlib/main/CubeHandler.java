...
public class CubeHandler {
	...
	protected CubeInfo getCubeInfo(int databaseId,int cubeId) throws PaloException{
		...
		String [][] onlyToken = HttpHandlerManager.getInstance().getHttpHandler(contextId).send(currentRequest,false,true);
	...
	private HashMap<String,Cell> extractCellsMapFromPath(StringBuilder request, String path, Dimension[]  cubeDims) throws PaloException, PaloJException {
		...
		for(int i=0;i<response.length;i++){
			if(response[i].length>3){
				...
				String [] pathIds = response[i][3].split(",");			
				Cell cell = new Cell(pathIds,response[i][2],(response[i][0].equals("1")?CellType.CELL_NUMERIC:CellType.CELL_STRING), cubeDims);
	...
	protected IRule[] getRules(Database database, int id) throws PaloException{
		...
		if(responses.length == 0)
			...
			for(int i=0;i<responses.length;i++){
				rules.add((IRule) new Rule(Integer.parseInt(responses[i][0]),responses[i][1],responses[i][2],responses[i][3],responses[i][4],responses[i][5]));
			}
	...
	protected void loadCells(int databaseId, int id, IElement[][] paths,Object[] values, int blockSize, boolean isAdd, SplashMode mode,boolean eventprocessor) throws PaloJException, PaloException {
		...
		for(;i<number_of_cells;){
			loadCellsFromIndex(databaseId, id, paths, values, blockSize, isAddInt, splashModeInt, i, eventprocessor);
			i+=blockSize;
		}
	...
	private void loadCellsFromIndex(int databaseId, int id, IElement[][] paths,Object[] values, int blockSize, int isAddInt, int splashMode,  int Startindex, boolean eventprocessor) throws PaloException {
		...
		for(int i=Startindex;i<max;i++){
			...
			String value = Helpers.urlEncode(Helpers.addDoubleQuotes(Helpers.escapeDoubleQuotes(values[i].toString())));
		...
		StringBuilder currentRequest = CELLS_REPLACE_BULK_REQUEST_BUFFER.append("cube=").append(id).append("&database=").append(databaseId).append("&paths=").append(pathsbuffer).append("&values=").append(valuebuffer).append("&add=").append(isAddInt).append("&splash=").append(splashMode).append("&event_processor=").append((eventprocessor?"1":"0"));
		HttpHandlerManager.getInstance().getHttpHandler(contextId).send(currentRequest,true,false);
	}
	...
	public Cell getCell(Database database, Cube cube, IElement[] path) throws PaloException, PaloJException {
		...
		return new Cell(pathIds,responses[0][2],(responses[0][0].equals("1")?CellType.CELL_NUMERIC:CellType.CELL_STRING), dimensions);			
	}
