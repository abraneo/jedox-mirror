...
public final class DatabaseHandler{
	...
	protected void addDimension(Database database, String name) throws PaloException, PaloJException{
		...
		StringBuilder currentRequest = CREATE_DIMENSION_REQUEST_BUFFER.append("new_name=").append(Helpers.urlEncode(Helpers.escapeDoubleQuotes(name))).append("&database=").append(database.getId()).append("&type=0");
	...
	protected LinkedHashMap<Key, Dimension> getDimensions(Database database) throws PaloException, PaloJException{
		...
		for(int i=0;i<responses.length;i++){
			...
			dimensions.put(new Key(responses[i][1].toLowerCase(),Integer.parseInt(responses[i][0])),new Dimension(contextId, Integer.parseInt(responses[i][0]),responses[i][1],database,type,responses[i][7],responses[i][8],responses[i][3],responses[i][4],responses[i][5],responses[i][10]));
		}
		return dimensions;
	}
	...
	protected void addCube(Database database, String name, int[] dimensionIds) throws PaloException, PaloJException{
		...
		StringBuilder currentRequest = CREATE_CUBE_REQUEST_BUFFER.append("new_name=").append(Helpers.urlEncode(Helpers.escapeDoubleQuotes(name))).append("&database=").append(database.getId()).append("&type=0").append("&dimensions=").append(dimensions);
		HttpHandlerManager.getInstance().getHttpHandler(contextId).send(currentRequest,true,false);
	}
	...
	protected LinkedHashMap<Key, Cube> getCubes(IDatabase database) throws PaloException, PaloJException{
		...
		for(int i=0;i<responses.length;i++){
			...
			cubes.put(new Key(responses[i][1].toLowerCase(),Integer.parseInt(responses[i][0])),new Cube(contextId, Integer.parseInt(responses[i][0]),responses[i][1],databaseObj));
		}
		return cubes;
	}
