...
public class DatabaseInfo extends ComponentInfo implements IDatabaseInfo{
	...
	private final DATABASE_STATUS status;
	...
	public DatabaseInfo(String cubesNum,String dimsNum,String status,String token){
		...
		case 0:
			this.status = DATABASE_STATUS.UNLOADED;
			break;
		case 1:
			this.status = DATABASE_STATUS.LOADED;
			break;
		case 2:
			this.status = DATABASE_STATUS.CHANGED;
			break;
			default:
	 ...
	 * @return the status
	 */
	public DATABASE_STATUS getStatus() {
		return status;
	}
