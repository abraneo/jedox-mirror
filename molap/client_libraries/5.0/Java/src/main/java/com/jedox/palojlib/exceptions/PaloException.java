...
public final class PaloException extends RuntimeException implements IPaloException {
	...
	public PaloException(String errorString) {
		...
		this.errorMsg = errorSections[1];
		this.errorDescription = errorSections[2];

		if(errorSections.length >3)
			this.errorReason = errorSections[3];
		else
			this.errorReason = "";
	}
	...
	public final String getMessage() {
		return errorMsg.replaceAll("\"", "") + "," + errorDescription.replaceAll("\"", "") + "(Palo error code: " + errorCode + ")";
	}
