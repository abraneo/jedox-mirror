 ...
 */
typedef std::vector<long> DIMENSION_LIST;

/*!
 * \brief
 * List of element ID's.
 */
typedef std::vector<long> ELEMENT_LIST;

typedef std::vector<long> GROUP_LIST;
...
class string_numeric : public std::string {
public:
	string_numeric();
	string_numeric(const std::string &str);
	string_numeric(double val);
	string_numeric(const string_numeric &sn);
	string_numeric &operator=(const string_numeric &sn);
	string_numeric &operator=(double value);
	string_numeric &operator=(const std::string &str);
	bool operator<(const string_numeric &str) const;
private:
	bool num;
	double val;
};
...
struct ELEMENT_INFO_EXT {
	string_numeric search_alias;
	std::string display_alias;
	ELEMENT_INFO m_einf;
	string_numeric sorting_alias;
	double value;
	...
	ELEMENT_INFO_EXT(const ELEMENT_INFO& einf) :
		m_einf(einf), value(0), cons_order(-1), curr_parent(0)
	{
	}

	ELEMENT_INFO_EXT() :
		value(0), cons_order(-1), curr_parent(0)
	{
	}

	inline bool operator<(const ELEMENT_INFO_EXT& e1) const
	{
		return (get_id() < e1.get_id());
	}
	...
	{
		return m_einf.element;
	}

	inline UINT get_position() const
	{
		return m_einf.position;
	}

	inline UINT get_idx(const unsigned int indent) const
	{
		return ((indent == 1) ? m_einf.indent : ((indent == 2) ? get_level() : m_einf.depth));
	}

};

typedef std::list<ELEMENT_INFO_EXT> ElementExList;
// End Subsets
