...
struct CubeHelper {
	...
	static const long getElementID(boost::shared_ptr<const SIElements> cache, const std::string& element_name);
	...
	static std::vector<std::vector<long> > getCoordIDs(boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, const DIMENSION_LIST& dimension_ids, const std::vector<std::vector<std::string> > &coords);
...
public:
	 ...
	 */
	Cube(boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, unsigned int cube);
	 ...
	 *                   SPLASH_MODE.
	 */
	bool CellReplace(const std::vector<long>& elements, const CELL_VALUE& cellvalue, SPLASH_MODE splashmode);
	 ...
	 *  @param eventprocessor : if false, the eventprocessor will not be called
	 */
	bool CellReplace(const std::vector<long>& elements, const CELL_VALUE& cellvalue, SPLASH_MODE splashmode, bool add, bool eventprocessor);
	...
	void CellExport(std::vector<CELL_VALUE_EXPORTED> &result, const std::vector<std::vector<long> > &coordinates, unsigned long blocksize, const std::vector<long> &start, const std::string condition, unsigned short BaseOnly = 0, unsigned short SkipEmpty = 1, unsigned short use_rules = 0);
	void DataFeedExport(std::vector<CELL_VALUE_EXPORTED> &result, unsigned long blocksize, const std::vector< long >& start, unsigned short SkipEmpty = 1 );
	...
	void MDXCellDrillThrough(std::vector<DRILLTHROUGH_INFO> &result, std::vector<long> elemids, DRILLTHROUGH_TYPE mode);
	 ...
	 */
	bool Commit(unsigned long LockID);
	 ...
	 *  @param LockID : Identifier of the locked area
	 */
	bool Rollback(unsigned long LockID);
	 ...
	 */
	bool Rollback(unsigned long LockID, long steps);
	...
	inline void MDXCellValue(CELL_VALUE &result, const std::vector<long> & elemids)
	{
		CellValue(result, elemids, 0, 0, false);
	}
	;
	inline void MDXCellValues(std::vector<CELL_VALUE> & res, const std::vector<std::vector<long> > & coord)
	{
	 ...
	 */
	RULE_INFO RuleCreate(const std::string& definition, unsigned short use_identifier = 0, std::string extern_id = "", std::string comment = "", unsigned short activate = 1);
	 ...
	 *  @return id , representation, extern_id, comment and timestamp of the rule
	 */
	RULE_INFO RuleModify(long id, const std::string& definition = "", unsigned short use_identifier = 0, std::string extern_id = "", std::string comment = "", unsigned short activate = 1);
	 ...
	 */
	std::vector<RULE_INFO> RulesActivate(const std::vector<long> &ruleIds, unsigned short activate = 1);
	 ...
	 */
	RULE_INFO RuleInfo(long id, unsigned short use_identifier = 0);
	 ...
	 * @return success
	 */
	bool RuleDestroy(const long identifier);
	 ...
	 * @return success
	 */
	bool RulesDestroy(const std::vector<long> &ruleIds);
...
private:
	...
	void CellValue(CELL_VALUE &result, const std::vector<long> & elemids, unsigned short showRule, unsigned short showLockState, bool ForceServerCall);
	void CellValues(std::vector<CELL_VALUE> & res, const std::vector<std::vector<long> > & coord, unsigned short showRule, unsigned short showLockState);
	void CellArea(std::vector<CELL_VALUE_PATH>& res, const std::vector<std::vector<long> > & coord, unsigned short showRule = 0, unsigned short showLockState = 0);
	bool CellCopy(const std::vector<std::string>& path, const std::vector<std::string>& path_to, double *value, bool userule, const std::vector<std::vector<std::string> > &lockedCoordinates);
};
