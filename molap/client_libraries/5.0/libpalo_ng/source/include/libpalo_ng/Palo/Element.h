...
public:
	 ...
	 */
	Element(boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, unsigned int dim, unsigned int element, boost::shared_ptr<const SIElements> cache);
	 ...
	 *  @param weights : Weights of the new children
	 */
	void append(const std::vector<long>& children, const std::vector<double>& weights);
	 ...
	 */
	void move(unsigned int position);
