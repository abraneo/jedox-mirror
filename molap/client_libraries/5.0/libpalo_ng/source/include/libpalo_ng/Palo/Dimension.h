...
public:
	Dimension(boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, unsigned int dim);
	 ...
	 */
	Element operator[](unsigned int id);
	 ...
	 */
	bool Exists(unsigned int id, bool cacheonly = false);
	 ...
	 */
	void createElement(ELEMENT_INFO& result, const std::string& Name, ELEMENT_INFO::TYPE type, const std::vector<long> &children, const std::vector<double> &weights);
	 ...
	 *  @param weights : List of weights
	 */
	void createElement(const std::string& Name, ELEMENT_INFO::TYPE type, const std::vector<long> &children, const std::vector<double> &weights);
	 ...
	 *  @param elements : List of element names.
	 */
	void bulkDeleteElements(const std::vector<long> &elements);
	...
	std::list<ELEMENT_INFO> getElements(long parentid = -1, long start = 0, long limit = -1) const;
...
private:
	std::unique_ptr<std::istringstream> getElementsIntern(long parentid, long start, long limit) const;
