...
Element::Element(boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, unsigned int dim, unsigned int element, boost::shared_ptr<const SIElements> cache) :
	m_ServerImpl(serverImpl), m_PaloClient(paloClient), m_Database(db), m_Dimension(dim), m_Element(element), m_Cache(!cache ? serverImpl->getElements(m_PaloClient, db, dim, false) : cache)
{
}
...
void Element::append(const std::vector<long>& children, const std::vector<double>& weights)
{
