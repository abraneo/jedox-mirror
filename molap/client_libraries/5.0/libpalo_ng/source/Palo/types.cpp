...
string_numeric::string_numeric(const std::string &str) :
	std::string(str), num(false), val(0)
{
}

string_numeric::string_numeric(double val) :
	num(true), val(val)
{
...
string_numeric::string_numeric(const string_numeric &sn) :
	std::string(sn), num(sn.num), val(sn.val)
{
}

string_numeric &string_numeric::operator=(const string_numeric &sn)
{
	std::string::operator=(sn);
	num = sn.num;
	val = sn.val;
	return *this;
}

string_numeric &string_numeric::operator=(double value)
{
	num = true;
	val = value;
	clear();
...
string_numeric &string_numeric::operator=(const std::string &str)
{
	num = false;
	val = 0;
...
bool string_numeric::operator<(const string_numeric &str) const
{
	if (num && str.num) {
		return val < str.val;
	} else {
		return util::UTF8Comparer::compare(*this, str) < 0;
	}
}
