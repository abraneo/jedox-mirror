...
struct ServerPool::ServerPoolImpl {
	...
	boost::mutex m_Lock;
	std::map<std::string, std::pair<boost::shared_ptr<ServerImpl>, int> > m_cache;
	...
	std::map<std::string, std::string> m_sid2id;
};
...
std::string ServerPool::adoptServer(const std::string& host, const unsigned int port, const std::string& sid, ServerProtocol protocol)
{
	...
	std::map<std::string, std::string>::iterator it = m_ServerPoolImpl->m_sid2id.find(sidKey.str());
...
void ServerPool::disconnectServer(const std::string& key, bool force)
{
	...
	if (its == m_ServerPoolImpl->m_server.end()) {
		...
		if (ita != m_ServerPoolImpl->m_alias2id.end()) {
			...
			userKey << its->second->m_PaloClient->getHost() << its->second->m_PaloClient->getPort() << its->second->m_PaloClient->getUsernameCapital();
			std::map<std::string, std::pair<boost::shared_ptr<ServerImpl>, int> >::iterator itc = m_ServerPoolImpl->m_cache.find(userKey.str());
...
std::string ServerPool::connectIntern(boost::shared_ptr<PaloClient> pc, bool connect, ServerProtocol protocol)
{
	...
	std::stringstream userKey;
	userKey << pc->getHost() << pc->getPort() << pc->getUsernameCapital();
	std::map<std::string, std::pair<boost::shared_ptr<ServerImpl>, int> >::iterator it = m_ServerPoolImpl->m_cache.find(userKey.str());
