 ...
 */
const long CubeHelper::getElementID(boost::shared_ptr<const SIElements> cache, const std::string& element_name)
{
...
ELEMENT_LIST CubeHelper::getElementIDs(boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, const DIMENSION_LIST& dimension_ids, const ELEMENT_NAME_LIST& element_names)
{
	...
	for (DIMENSION_LIST::size_type i = 0; i < dim_ids_size; ++i) {
		return_vec.push_back(getElementID(serverImpl->getElements(paloClient, db, dimension_ids[i], false), element_names[i]));
	}
	return return_vec;
}
...
ELEMENT_LIST CubeHelper::getElementIDs(boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, const unsigned int dimension_id, const ELEMENT_NAME_LIST& element_names)
{
	...
	boost::shared_ptr<const SIElements> dim = serverImpl->getElements(paloClient, db, dimension_id, false);
...
std::vector<std::vector<long> > CubeHelper::getCoordIDs(boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, const DIMENSION_LIST& dimension_ids, const std::vector<std::vector<std::string> > &coords)
{
	std::vector<std::vector<long> > return_vec;
...
void CubeHelper::coordinates2query(const std::vector<std::vector<std::string> > & coordinates, std::stringstream &query, boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, const DIMENSION_LIST& dimlist)
{
	...
	for (DIMENSION_LIST::const_iterator it = dimlist.begin(); it != dimlist.end(); ++it) {
		elemlists.push_back(serverImpl->getElements(paloClient, db, *it, false));
	}
...
Cube::Cube(boost::shared_ptr<ServerImpl> serverImpl, boost::shared_ptr<PaloClient> paloClient, unsigned int db, unsigned int cube) :
	m_ServerImpl(serverImpl), m_PaloClient(paloClient), m_Dat(db), m_Cube(cube), m_Cache(serverImpl->getCubes(m_PaloClient, db, false))
{
}
...
void Cube::clear(const std::vector<std::vector<std::string> > & coordinates)
{
	...
	std::vector<std::vector<long> > coord(CubeHelper::getElementIDs(m_ServerImpl, m_PaloClient, m_Dat, dimlist, coordinates));
...
bool Cube::CellReplaceWithLock(const std::vector<std::string> & elements, const CELL_VALUE& cellvalue, SPLASH_MODE splashmode, bool add, bool eventprocessor, const std::vector<std::vector<std::string> > &lockedCoordinates)
{
	...
	std::vector<long> elemids(CubeHelper::getElementIDs(m_ServerImpl, m_PaloClient, m_Dat, dimlist, elements));
...
bool Cube::CellReplace(const std::vector<long> & elements, const CELL_VALUE& cellvalue, SPLASH_MODE splashmode, bool add, bool eventprocessor)
{
	char result = 0;
	const CubeCache &cube = (*m_Cache)[m_Cube];
	const DIMENSION_LIST& dimlist = cube.dimensions;
...
bool Cube::CellReplace(const std::vector<long> & elements, const CELL_VALUE& cellvalue, SPLASH_MODE splashmode)
{
	return Cube::CellReplace(elements, cellvalue, splashmode, false, true);
}
...
void Cube::CellValue(CELL_VALUE &result, const std::vector<long> & elemids, unsigned short showRule, unsigned short showLockState, bool ForceServerCall)
{
	const CubeCache &cube = (*m_Cache)[m_Cube];
	unsigned int dummy, sequencenumber = cube.getSequenceNumber();
	CUBE_TOKEN token(sequencenumber);
...
void Cube::CellArea(std::vector<CELL_VALUE_PATH>& res, const std::vector<std::vector<long> > & coord, unsigned short showRule, unsigned short showLockState)
{
	std::stringstream query;
...
void Cube::CellValues(std::vector<CELL_VALUE> & res, const std::vector<std::vector<long> > & coord, unsigned short showRule, unsigned short showLockState)
{
	std::stringstream query;
...
void Cube::CellValues(std::vector<CELL_VALUE> & res, const std::vector<std::vector<std::string> > & coordinates, unsigned short showRule, unsigned short showLockState)
{
	...
	elemlists.reserve(dimlist.size());
	for (DIMENSION_LIST::const_iterator it = dimlist.begin(); it != dimlist.end(); ++it) {
		elemlists.push_back(m_ServerImpl->getElements(m_PaloClient, m_Dat, *it, false));;
	}
	size_t vsize = coordinates.size(), vsize2, vsize3, dsize = dimlist.size();
	std::vector<long> elemids;
	std::vector<std::vector<long> > coord(vsize);
...
void Cube::CellExport(std::vector<CELL_VALUE_EXPORTED> &result, const std::vector<std::vector<long> > &coordinates, unsigned long blocksize, const std::vector<long> &start, const std::string condition, unsigned short BaseOnly, unsigned short SkipEmpty, unsigned short use_rules)
{
	const CubeCache &cube = (*m_Cache)[m_Cube];
	const DIMENSION_LIST& dimlist = cube.dimensions;
...
void Cube::DataFeedExport( std::vector<CELL_VALUE_EXPORTED> &result, unsigned long blocksize, const std::vector< long >& start, unsigned short SkipEmpty )
{
	const CubeCache &cube = (*m_Cache)[m_Cube];
	const DIMENSION_LIST& dimlist = cube.dimensions;
...
bool Cube::CellCopy(const std::vector<std::string> & path, const std::vector<std::string> & path_to, double *value, bool userule, const std::vector<std::vector<std::string> > &lockedCoordinates)
{
	...
	const CubeCache &cube = (*m_Cache)[m_Cube];
	const DIMENSION_LIST& dimlist = cube.dimensions;
	size_t vsize = path.size(), vsize2 = path_to.size(), dsize = dimlist.size();

	if ((vsize != dsize) || (vsize2 != dsize)) {
		const unsigned int errorcode = PaloExceptionFactory::ERROR_INVALID_COORDINATES;
		const std::string errormsg = WRONGDIMENSIONCOUNT;
		throw PaloServerException(errormsg, errormsg, errorcode);
	}

	std::vector<long> source(CubeHelper::getElementIDs(m_ServerImpl, m_PaloClient, m_Dat, dimlist, path));
	std::vector<long> target(CubeHelper::getElementIDs(m_ServerImpl, m_PaloClient, m_Dat, dimlist, path_to));
	...
	query << "database=" << m_Dat << "&cube=" << m_Cube << "&path=";
	jedox::util::TListe(query, source, ',', false);
	query << "&path_to=";
	jedox::util::TListe(query, target, ',', false);
	if (value) {
		query << "&value=" << *value;
	}
	if (userule) {
...
void Cube::MDXCellDrillThrough(std::vector<DRILLTHROUGH_INFO> &result, std::vector<long> elemids, DRILLTHROUGH_TYPE mode)
{
	const CubeCache &cube = (*m_Cache)[m_Cube];
	const DIMENSION_LIST& dimlist = cube.dimensions;
...
RULE_INFO Cube::RuleCreate(const std::string& definition, unsigned short use_identifier, std::string extern_id, std::string comment, unsigned short activate)
{
	RULE_INFO result;
	std::stringstream query;
...
RULE_INFO Cube::RuleModify(long id, const std::string& definition, unsigned short use_identifier, std::string extern_id, std::string comment, unsigned short activate)
{
	RULE_INFO result;
	std::stringstream query;
...
std::vector<RULE_INFO> Cube::RulesActivate(const std::vector<long> &ruleIds, unsigned short activate)
{
	RULE_INFO result;
...
RULE_INFO Cube::RuleInfo(long id, unsigned short use_identifier)
{
	RULE_INFO result;
	std::stringstream query;
...
bool Cube::RuleDestroy(const long identifier)
{
	char result = 0;
	std::stringstream query;
...
bool Cube::RulesDestroy(const std::vector<long> &ruleIds)
{
	char result = 0;
	std::stringstream query;
...
bool Cube::Commit(unsigned long LockID)
{
	char result = 0;
	std::stringstream query;
...
bool Cube::Rollback(unsigned long LockID)
{
	return Rollback(LockID, -1);
}

bool Cube::Rollback(unsigned long LockID, long steps)
{
	char result = 0;
	std::stringstream query;
