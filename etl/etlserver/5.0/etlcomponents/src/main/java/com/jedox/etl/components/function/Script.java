...
public abstract class Script extends Function {
	...
	protected Object transform(Row values) throws FunctionException {
		try {
			...
			return typeConversion.convert(convertColumn);
		}
	...
	protected void validateParameters() throws ConfigurationException {
		...
		IManager manager = APIExtender.getInstance().getGuessedDependencies(this, getScript(), getContextName());
		addManager(manager);
		String sources = getParameter("sources",null);
		if (sources != null) {
			for (String sourceName : sources.split(",")) {
				try {
					manager.add(ConfigManager.getInstance().getComponent(new Locator().add(getLocator().getRootName()).add(ITypes.Sources).add(sourceName.trim()), getContextName()));
				}
				catch (Exception e) {
