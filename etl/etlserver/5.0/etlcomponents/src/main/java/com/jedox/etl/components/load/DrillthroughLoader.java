...
public class DrillthroughLoader{
	...
	public void addDrillthroughPersistor() throws RuntimeException{
		...
		if (drillthrough != null) {
				...
				if(drillthrough.drillThroughExternalBulksSize!=-1)
					definition.setBulkSize(drillthrough.drillThroughExternalBulksSize);
				...
				for (IColumn col : rows.current().getColumns()) {
					...
					definition.setType(col.getName(), String.class.getCanonicalName());
				}
				checkAnnexColumns();

				//define value
				IColumn valueColumn = rows.current().getColumn(rows.current().size()-1);
				if (valueColumn.getElementType().equals(ElementType.ELEMENT_NUMERIC)) {
					definition.setRole(valueColumn.getName(), PersistenceUtil.getDataPersistenceMode(writeMode));
					definition.setType(valueColumn.getName(), Double.class.getCanonicalName());
				}
				else {
					definition.setRole(valueColumn.getName(), UpdateModes.last);
					definition.setType(valueColumn.getName(), String.class.getCanonicalName());
				}
