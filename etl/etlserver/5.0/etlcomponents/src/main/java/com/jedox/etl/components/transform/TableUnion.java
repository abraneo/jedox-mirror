...
public class TableUnion extends TableSource implements ITransform {
	...
	private SourceManager baseManager = new SourceManager();
	private SourceManager loopManager = new SourceManager();
	...
	protected IProcessor getSourceProcessor(int size) throws RuntimeException {
		...
		if (!loopManager.isEmpty()) { //there are loops
			IProcessor loops = UnionProcessor.getInstance(loopManager.getProcessors());
			bases = new LoopProcessor(loops,baseManager);
		}
		else { //no loops, just a union
			bases = UnionProcessor.getInstance(baseManager.getProcessors());
		}
