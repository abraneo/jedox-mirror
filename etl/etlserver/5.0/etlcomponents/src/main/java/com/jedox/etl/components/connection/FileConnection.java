...
public class FileConnection extends RelationalConnection implements IFileConnection {
	...
	protected synchronized static Filestore ensurePersistence(FileConnection connection, int size) throws RuntimeException {
		PersistorDefinition definition = connection.getStoreDefinition();
		Filestore store = getStore(definition);
		File file = new File(connection.getFilename());
		if (file.lastModified() >= store.getTimestamp()) {
			log.debug("Caching File Connection "+connection.getName()+" to internal persistence (lines: "+size+")");
			IProcessor processor = connection.getFileProcessor(new AliasMap(), size);
			...
			try {
				...
				if (size <= 0) { //only consider full import as valid cache.
	...
	protected class FileProcessor extends Processor {
		...
		private CSVReader getReader() throws RuntimeException {
			CSVReader reader;
			try {
				InputStream s = FileUtil.getInputStream(filename);
				char sep;
				if (delimiter.isEmpty())
					sep = ' ';
				else {
					sep = delimiter.toCharArray()[0];
					if (delimiter.equals("\\t"))
						sep = '\t';
				}
				reader = new CSVReader(new InputStreamReader(s,encoding), sep, readingQuote.toCharArray()[0], escapeQuote);
			}
			catch (Exception e) {
				throw new RuntimeException("Failed to read file "+new File(getFilename()).getAbsolutePath()+": "+e.getMessage());
			}
			return reader;
		}
		...
		private Row getHeader(String[] line, int columns) throws RuntimeException {
			...
			for (int i=0; i<columns;i++) {
				...
				if ((header) && (i < line.length)) {
					...
					String headerName = recoder.recode(line[i].trim());
			...
			for (int i=0; i<origNames.size(); i++) {
				row.setOriginalName(i, origNames.get(i));
			}
			return row;
		}
		...
		protected boolean fillRow(Row row) throws Exception {
			...
			// Log file processor progress each <logLines> rows
			// ToDo: Do generic solution in Processor for all extracts
			count++;
			if (logLines != 0 && count % logLines == 0)
				log.info("Read "+count+" lines of file "+filename);
...
//		}
	...
	public void test() throws RuntimeException {
		checkSSL();
		FileUtil.test(getFilename(), true);		
	}
	...
	protected IProcessor getFileProcessor(IAliasMap aliasMap, int size) throws RuntimeException {
		IProcessor processor = new FileProcessor(getFilename(),getFileEncoding(), aliasMap);
	...
	protected String getHash() throws RuntimeException {
		...
		result.append(getContext().getBaseContextName());
	...
	public void init() throws InitializationException {
		try {
			...
			logLines = Integer.valueOf(getParameter("logLines","0"));
	...
	private StringWriter getColumnNamesWriter() throws RuntimeException{
...
//			for (String columnName : firstRow.getColumnNames()) {
...
//			}
	...
	public String getMetadata(Properties properties) throws RuntimeException {
		...
		if (selector.equals("columns")) 
			return getColumnNamesWriter().toString();
		else
