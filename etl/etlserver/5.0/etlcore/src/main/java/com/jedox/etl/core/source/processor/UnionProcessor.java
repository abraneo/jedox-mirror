		...
		for (int i=0; i<processors.size(); i++) {
			...
			for (IColumn source : p.current().getColumns()) {
				...
				if (target == null) { //column is not yet present in internal row
					CoordinateNode cn = new CoordinateNode(source.getName());
					cn.setInput(source);
					row.addColumn(cn);
					...
					if (!target.getElementType().equals(source.getElementType()))
						log.warn("Column "+source.getName()+" of source "+p.getName()+" is of element type "+source.getElementType()+". Type "+target.getElementType()+" was set for this union by a previous source.");
				}
			}
		}
	}
