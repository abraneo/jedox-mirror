...
public class XMLTreeManager implements IDimension {
	...
	private Element createElement(String name) {
		...
		try {
			elementCount++;
		    element = manager.createElement(name);
		}
		catch (DOMException e) {
			String newName = null;
			try {
				newName = "etl_"+name.replace(" ", "");
				log.warn("Changing illegal tag name "+name+" to "+newName+".");
				element = manager.createElement(newName);
			}
			catch (DOMException e1) {
				newName = "etl_"+elementCount;
				log.warn("Changing illegal tag name etl_"+name+" to "+newName+".");
				element = manager.createElement(newName);
			}
	...
	public void addAttributes(String[] names, ElementType[] types)
		...
		for (int i = 0; i < names.length; i++) {
			XMLAttribute column = new XMLAttribute();
