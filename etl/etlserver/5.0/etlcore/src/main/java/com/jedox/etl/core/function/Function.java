...
public abstract class Function extends Component implements IFunction {
	...
	protected static enum Types {
		tDouble, tInteger, tLong, tByte, tFloat, tShort, tString, tObject, tint, tdouble, tbyte, tfloat, tshort, tlong
	}
	...
	protected TypeConversionUtil typeConversion = new TypeConversionUtil();
	 ...
	 */
	protected String getClassnameForType(String inputType) throws ConfigurationException {
		String name;
		Types t = Types.valueOf("t"+inputType);
		switch (t) { //first test for primitives then for classes.
			case tint: name = Integer.class.getCanonicalName(); break;
			case tInteger: name = Integer.class.getCanonicalName(); break;
			case tdouble: name = Double.class.getCanonicalName(); break;
			case tDouble: name = Double.class.getCanonicalName(); break;
			case tshort: name = Short.class.getCanonicalName(); break;
			case tShort: name = Short.class.getCanonicalName(); break;
			case tlong: name = Long.class.getCanonicalName(); break;
			case tLong: name = Long.class.getCanonicalName(); break;
			case tbyte: name = Byte.class.getCanonicalName(); break;
			case tByte: name = Byte.class.getCanonicalName(); break;
			case tString: name = String.class.getCanonicalName(); break;	
			case tfloat: name = Float.class.getCanonicalName(); break;
			case tFloat: name = Float.class.getCanonicalName(); break;	
			case tObject: name = Object.class.getCanonicalName(); break;	
	 ...
	 * @throws ConfigurationException
	 */
	protected String[] getClassnamesForTypes(String[] inputTypes) throws ConfigurationException {
		String[] names = new String[inputTypes.length];
		for (int i=0; i<inputTypes.length; i++) {
			if (inputTypes[i]==null) {
				names[i] = null;
			} else {
	...
	protected Object convert(IColumn column) throws FunctionException {
		try {
			return typeConversion.convert(column);
		}
		catch (Exception e) {
	...
	public final Object getValue() {
		try {
			Object value = null;
			if (isCached()) {
				value = lookup.get(getKey(getInputs()));
			}
			if (value == null) {
				value = transform(getInputs());
				counter++;
				setValue(value);
			} 
			return value;
		...
		/*
			...
			getLog().warn("Unexpected exception caught in Function "+getName()+": "+e.getClass().getCanonicalName()+" - "+e.getMessage());
		}
		*/
		return null;
	}
	...
	public void setElementType(String type) {
		output.setElementType(type);
	}
	
	public int getScale() {
		return output.getScale();
	}
	...
	try {
			...
			String[] inputClassNames = getClassnamesForTypes(inputTypes);
			for (int j=0; j<inputNames.length; j++) {
				String inputName = inputNames[j];
				String inputValue = inputValues[j];
				String inputClassName = inputClassNames[j];
				CoordinateNode column = new CoordinateNode(inputName);
				...
				if (inputValue != null){ //a constant
					...
					column.setConstantValue(inputValue);
				}
				addInput(column);
			}
