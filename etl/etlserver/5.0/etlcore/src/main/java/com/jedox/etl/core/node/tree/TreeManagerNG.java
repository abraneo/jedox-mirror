...
public class TreeManagerNG implements ITreeManager {
	...
	private Map<String,List<INamedValue<Object>>> attributeBuffer = new HashMap<String,List<INamedValue<Object>>>();
	...
	public Element provideElement(String name, ElementType type) throws PaloJException {
		...
		if (result == null) {
			...
			if (!result.getType().equals(type) && !result.getType().equals(ElementType.ELEMENT_CONSOLIDATED)) {
	...
	private Attribute getAttributeDefinition(String name) {
	...
	private Attribute findAttributeDefinition(String name) throws PaloJException {
		Attribute attributeColumn = getAttributeDefinition(name);
	...
	private Attribute addAttribute(String name, ElementType type, boolean override) throws PaloJException {
		...
		if (!override) {
			Attribute existing = getAttributeDefinition(a.getName());
	...
	public void addSubtree(IElement subTreeRoot, IConsolidation[] consolidations) throws PaloJException {
		...
		//cschw: this is not necessary and causes elements to appear on root level, which have merge nodes.
	...
	public Attribute getAttributeByName(String name) throws PaloException, PaloJException {
		return getAttributeDefinition(name);
	}
	...
	public Object getAttributeValue(String attributeName, String elementName, boolean verify) throws PaloJException {
		...
		if (verify) {
			...
			if (value == null && a.getMode().equals(AttributeModes.ALIAS)) value = e.getName();
			if (a.getType().equals(ElementType.ELEMENT_NUMERIC) && value != null) {
				boolean numeric = new TypeConversionUtil().isNumeric(value.toString());
				if (!numeric) {
					log.warn("Attribut "+a.getName()+" of Element "+elementName+" is of type numeric, but attribut value is not numeric: "+value.toString());
				...
				if (parents != null) {
					...
					for (IConsolidation consolidation : parents) {
						...
						if (isCyclic(consolidation) || !newParents.add(p)) { //remove consolidation, if it is cyclic or duplicate
							childrenLookup.get(consolidation.getParent()).remove(consolidation);
						} else {
