...
public class Locator implements Cloneable {
	...
	private static String complexsuffix = "..";
	...
	public String getPersistentName(String quote) {
		return ("".equals(getPersistentSchema())) ? NamingUtil.escape(getName(),quote) : NamingUtil.escape(getPersistentSchema(),quote)+"."+NamingUtil.escape(getName(),quote);
	}
	...
	/**
	 * gets the locator of a string which includes the referenced component type as suffix
	 * @param locatorComplex  
	 * @return
	 */
	public static Locator parseComplex (String locatorComplex) {
		int index = locatorComplex.indexOf(complexsuffix);
		if (index==-1)
			return parse(locatorComplex);
		else
			return parse(locatorComplex.substring(0, index));
	}	

	/**
	 * gets the string representation of a locator with the referenced component type as suffix to distinguish 2 locators used in different contexts 
	 * @param concatLocator
	 * @return
	 */
	public String toStringComplex (String type) {
