...
public class AttributesUtil {
	private ColumnManager columns = new ColumnManager();
	
	public AttributesUtil(Row attributeRow) {
		for (IColumn attribute : attributeRow.getColumns()) {
			if (!attribute.getName().startsWith(NamingUtil.internalPrefix())){
				ColumnNode a = new ColumnNode(attribute.getName());
				a.mimic(attribute);
				columns.addColumn(a, attribute.getColumnType());
			}
		}
	}
	...
	public ColumnNode fillAttribute(INamedValue<?> node, int pos, ITreeManager manager) {
		...
		c.setConstantValue(attributeValue != null ? attributeValue : "");
		return c;
	}
