...
public class HibernateAggregator extends HibernatePersistor {
	...
	private boolean isBulkAggregable(PersistorDefinition definition) {
		...
		if (MappingHelper.strategyAssigned.equals(definition.getPrimaryKeyGeneration()) && getPrimaryKeys().size() == 1) {
			...
			try {
				if (Number.class.isAssignableFrom(Class.forName(c.getValueType()))) {
	...
	private String getSQLAggregateSelect(IColumn c, String quote) {
		String name = SQLUtil.quoteName(c.getName(), quote);
		String aggName =  SQLUtil.aliasName(c.getColumnType().toString()+"("+name+")",name);
	...
	private String getSQLAggregateQuery(Long offset) throws RuntimeException  {
		...
		ColumnManager toSet = new ColumnManager();
		...
		//join maxSource?
		if (toSet.getColumnsOfType(ColumnTypes.last).size() > 0) {
	...
	private Double getNumericValue(String name, Object value) {
		...
		String conv = getConverter().convertToNumeric(value.toString());
		if (getConverter().isNumeric(conv)) {
			return Double.valueOf(conv);
		}
	...
	private void rowAggregate(Row row) throws RuntimeException {
		try {
			...
			if (result.isEmpty()) {
				//check for count
				for (IColumn c : getDefinition().getToSet().getColumns()) {
					if (c.getColumnType().equals(ColumnTypes.count))
