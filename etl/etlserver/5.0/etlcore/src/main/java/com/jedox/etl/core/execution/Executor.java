...
public class Executor implements IExecutor {
	...
	public ExecutionState removeExecution(Long id, boolean removeFromHistory) throws ExecutionException {
		...
		try {
			e = getExecution(id);
			if (Codes.statusStopping.equals(e.getExecutionState().getStatus())) { //cschw: execution is stuck in stopping. lets kill it via interrupt
				Future<?> f = controller.getFuture(id);
				if (f != null) {
					if (f.cancel(true)) {
						state = stop(e,true); 
					}
			...
			else if(!removeFromHistory && !e.isActive()){
				 throw new ExecutionException("Execution can not be stoppped, only execution with status \"Running\", \"Queued\" or \"Stopping\" can be stopped.");
			}else{
				state = stop(e,false); //normal stop. just set executable flag to false and let components of execution handle it.
			}			
		}
	...
	public Execution createExecution(Locator locator, Properties properties) throws ExecutionException {
		return new Execution(ExecutionTypes.EXECUTION,locator,properties,new Properties());
	}
	...
	public Execution createExecution(Locator locator, Properties properties, ExecutionState masterState) throws ExecutionException {
		return new Execution(ExecutionTypes.EXECUTION,locator,properties,new Properties(),masterState);
	}
	...
	private ExecutionState stop(Execution execution, boolean interrupt) throws ExecutionException {
		if (interrupt)
			execution.getExecutionState().addMessageExternal("INFO", "Requested interruption of execution (ID: "+execution.getKey()+")");
		else
			execution.getExecutionState().addMessageExternal("INFO", "Requested stop of execution (ID: "+execution.getKey()+")");
		...
		return getStatus(execution,true);
	}
	...
	private ExecutionState getStatus(Execution execution, boolean waitForTermination) throws ExecutionException {
		...
		return execution.getExecutionState();
	}
