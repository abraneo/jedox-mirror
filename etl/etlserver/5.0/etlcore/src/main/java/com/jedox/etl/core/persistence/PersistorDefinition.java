...
public class PersistorDefinition {
	...
	public Row getColumnDefinition() {
		if (definition == null) {
			...
			for (int i=0; i<input.size(); i++) {
				...
				ColumnNode d = new ColumnNode(c.getName());
				d.setInput(c);
				UpdateModes role = getRole(c.getName());
				String dataType = getType(c.getName());
				if (dataType != null)
					d.setValueType(dataType);
				if (role != null) {
					//convert update modes to column modes
					d.setColumnType(ColumnTypes.valueOf(role.toString()));
				}
	...
	public Row getKeys() {
		if (keys == null) {
			keys = new Row();
			for (IColumn c : getColumnDefinition().getColumns()) {
				IColumn.ColumnTypes type = c.getColumnType();
				switch (type) {
