...
public abstract class Load extends Component implements ILoad {
	...
	protected IProcessor getProcessor() throws RuntimeException {
		...
		if(lastRow>= Integer.MAX_VALUE-1)
			rows.setLastRow(rows.getLastRow());
		else
			rows.setLastRow(Math.min(rows.getLastRow(),rows.getFirstRow()+lastRow));
		rows.setFirstRow(rows.getFirstRow()+ getFirstRow());
		rows.setEncoding(null, getEncoding());
		rows.setState(getState());
		return rows;
	}

	protected IProcessor getProcessor(IView.Views view) throws RuntimeException {
		IProcessor rows = getView().getProcessor(view);
		rows.setFirstRow(getFirstRow());
		rows.setLastRow(getLastRow());
		rows.setEncoding(null, getEncoding());
		rows.setState(getState());
		return rows;
	}
