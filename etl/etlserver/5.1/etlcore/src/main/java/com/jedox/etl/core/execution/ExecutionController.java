...
public class ExecutionController {
	...
	public void add(Execution execution) throws com.jedox.etl.core.execution.ExecutionException {
		...
		case STANDARD: syncModeValue = execution.getExecutable().getContext().getParameter().getProperty(NamingUtil.internal("syncMode"), configSyncModeString); break;
	...
	public void remove(Execution execution) {
		boolean directRemoved = direct.remove(execution);
		boolean parallelRemoved = parallel.remove(execution);
		boolean singleRemoved = single.remove(execution);
		if(directRemoved)
			direct.purge();
		if(parallelRemoved)
			parallel.purge();
