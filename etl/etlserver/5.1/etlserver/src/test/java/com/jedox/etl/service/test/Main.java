...
import com.jedox.etl.service.ExecutionDetailEntry;
...
public class Main {
	...
	private static void executeComponents(String[] locators) throws Exception {
		...
		for (ExecutionDescriptor d : descriptors) {
			...
			ExecutionDetailEntry[] eds = service.getExecutionDetails(d.getId(),null).getExecutionDetailentries();
			
			for (ExecutionDetailEntry ed : eds) {
				System.out.println(ed.getLocator()+" "+ed.getProcessedInputRows()+" "+ed.getProcessedOutputRows());
			}
	...
	private  static void getData(String locator, String format) throws Exception {
		//getComponentOutline
		ExecutionDescriptor s = service.getData(locator,new Variable[]{}, format,20,0,true,null);
	...
	private static void getComponentTypes(String scope) throws Exception {
		ComponentTypeDescriptor[] results = service.getComponentTypes(scope,null);
	...
	private static void testConfigManager() throws Exception {
		...
		ConfigManager.getInstance().add(Locator.parse("importOLAP"), stringTojdom(config[0].getResult()));
		//System.out.println(ConfigManager.getInstance().getConfiguration(Locator.parse("importOLAP")));
		ConfigManager.getInstance().removeElement(Locator.parse("importOLAP"));
	...
	private static void getComponentGraph(String locator) throws Exception {
		ComponentDependencyDescriptor[] in = service.getComponentDependencies(locator,false);
...
//		service.renameComponents(new String[]{"sampleBikerOrig.transforms.OrdersData_Trans.functions.Year"}, new String[]{"NewYear"},true);
		
//		getComponentConfigs(new String[]{"sampleBikerOrig"})[0].getResult();
		
//		System.out.println(service.getProjectDocumentation("importRelDB"));
...
/* Parallel execution test		
		...
		syncModeGroup.setValue(SyncModes.DIRECT.toString());
