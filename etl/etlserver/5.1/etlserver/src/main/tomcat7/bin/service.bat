...
set DISPLAYNAME=Jedox Suite Tomcat Service
...
echo Using java path library: %Library_Path%
...
:foundJvm
rem start JedoxSuite_CUSTOM_CONFIG
rem set JMX=-Dcom.sun.management.jmxremote;-Dcom.sun.management.jmxremote.port=9999;-Dcom.sun.management.jmxremote.ssl=false;-Dcom.sun.management.jmxremote.authenticate=false;
...
"%EXECUTABLE%" //IS//%SERVICE_NAME% ^
    ...
    --JvmMs %Min_Memory% ^
    --JvmMx %Max_Memory% ^
	--DependsOn JedoxSuiteMolapService ^
	--Startup auto
	
"%EXECUTABLE%" //US//%SERVICE_NAME% ^
	++JvmOptions  %JVM_Optional_Parameters%-XX:MaxPermSize=%PermGen_Max_Memory%m;-Djava.library.path="%Library_Path%";%JMX%%SECURITY_POLICY_PARAM%
